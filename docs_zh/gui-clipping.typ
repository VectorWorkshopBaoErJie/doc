= 裁剪
<裁剪>
GUI节点可以用作#emph[裁剪]节点------控制其他节点如何渲染的遮罩。本手册解释了此功能的工作原理。
...
/ Clipping Visible: #block[
勾选以渲染节点的内容。
]

/ Clipping Inverted: #block[
勾选以将节点形状的反转写入蒙版。
]

然后将要裁剪的节点作为子节点添加到裁剪节点中。

#box(image("../docs/en/manuals/images/gui-clipping/create.png"))

== 模板蒙版
<模板蒙版>
裁剪的工作原理是让节点写入#emph[模板缓冲区]。此缓冲区包含裁剪蒙版：告诉显卡像素是否应该被渲染的信息。

- 没有裁剪器父节点但裁剪模式设置为`Stencil`的节点会将其形状（或其反转形状）写入存储在模板缓冲区中的新裁剪蒙版。
- 如果裁剪节点有裁剪器父节点，它将裁剪父节点的裁剪蒙版。裁剪子节点永远不能\_扩展\_当前裁剪蒙版，只能进一步裁剪它。
- 作为裁剪器子节点的非裁剪器节点将使用由父层级创建的裁剪蒙版进行渲染。

#box(image("../docs/en/manuals/images/gui-clipping/setup.png"))
...
#box(image("../docs/en/manuals/images/gui-clipping/modes.png"))

== 模板限制
<模板限制>
- 模板裁剪器的总数不能超过256个。
- 子\_模板\_节点的最大嵌套深度为8层。（仅计算具有模板裁剪的节点。）
- 模板节点的同级节点最大数量为127个。对于模板层级中的每一层，最大限制减半。
- 反转节点的成本更高。反转裁剪节点的限制为8个，每个节点将使非反转裁剪节点的最大数量减半。
- 模板从节点的\_几何形状\_（而不是纹理）渲染模板蒙版。可以通过设置#emph[Inverted clipper]属性来反转蒙版。
...
#box(image("../docs/en/manuals/images/gui-clipping/layers.png"))
