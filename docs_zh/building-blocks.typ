...
#box(image("../docs/en/manuals/images/building_blocks/building_blocks.png"))
...
当Defold引擎启动时，它会加载一个在#emph[game.project]设置文件中指定的\_引导集合\_。引导集合通常命名为”main.collection”，但您可以自由使用任何您喜欢的名称。
...
#box(image("../docs/en/manuals/images/building_blocks/collection.png"))

请注意，ID为”bean”的子集合存储在自己的文件中，名为”/main/bean.collection”，仅在”main.collection”中被引用：

#box(image("../docs/en/manuals/images/building_blocks/bean_collection.png"))

您无法对集合本身进行寻址，因为没有与”main”和”bean”集合对应的运行时对象。但是，您有时需要使用集合的标识作为游戏对象\_路径\_的一部分（详细信息请参阅定位手册）：
...
在#emph[Outline]视图中右键单击集合并选择Add Collection File。
...
游戏对象可以空着使用（例如作为位置标记），但通常配备各种组件，如精灵、声音、脚本、模型、工厂等。游戏对象要么在编辑器中创建，放置在集合文件中，要么在运行时通过\_factory\_组件动态生成。

游戏对象可以直接添加到集合中，或者作为对游戏对象文件的引用添加到集合中：

在#emph[Outline]视图中右键单击集合并选择Add Game Object（就地添加）或Add Game Object File（作为文件引用添加）。
...
== 就地添加或通过引用添加的对象
<就地添加或通过引用添加的对象>
当您创建集合、游戏对象或组件\_文件\_时，您创建的是我们所说的原型（在其他引擎中也称为”预制件”或”蓝图”）。这只是在项目文件结构中添加了一个文件，并没有在运行的游戏中添加任何内容。要添加基于原型文件的集合、游戏对象或组件的实例，您需要在其中一个集合文件中添加它的实例。

您可以在大纲视图中看到对象实例基于哪个文件。"main.collection"文件包含三个基于文件的实例：

+ "bean"子集合。
+ "bean"子集合中”bean”游戏对象中的”bean”脚本组件。
+ "can"游戏对象中的”can”脚本组件。

#box(image("../docs/en/manuals/images/building_blocks/instance.png"))

当您有多个游戏对象或集合的实例并希望更改所有实例时，创建原型文件的好处就变得明显：

#box(image("../docs/en/manuals/images/building_blocks/go_instance.png"))

通过更改原型文件，任何使用该文件的实例都将立即更新。

#box(image("../docs/en/manuals/images/building_blocks/go_change_blueprint.png"))

这里原型文件的精灵图像被更改，所有使用该文件的实例立即更新：

#box(image("../docs/en/manuals/images/building_blocks/go_instance2.png"))

== 创建游戏对象的父子关系
<创建游戏对象的父子关系>
在集合文件中，您可以构建游戏对象的层次结构，使一个或多个游戏对象成为单个父游戏对象的子对象。只需简单地拖动一个游戏对象并将其放置到另一个游戏对象上，被拖动的游戏对象就成为目标游戏对象的子对象：

#box(image("../docs/en/manuals/images/building_blocks/childing.png"))

对象父子层次结构是一种动态关系，影响对象如何响应变换。在编辑器和运行时，应用于对象的任何变换（移动、旋转或缩放）都会依次应用于对象的子对象：

#box(image("../docs/en/manuals/images/building_blocks/child_transform.png"))
