...
OpenGL ES 2.0（嵌入式系统 OpenGL）和 OpenGL ES 着色语言的规范可在 Khronos OpenGL 注册表中找到。
...
当模型放置在游戏世界中时，模型的局部顶点坐标必须转换为世界坐标。这种转换是通过#emph[世界变换矩阵]完成的，它告诉应该对模型的顶点应用什么转换（移动）、旋转和缩放，以便正确放置在游戏世界的坐标系中。
#box(image("../docs/en/manuals/images/shader/world_transform.png"))
]

/ 视图和投影矩阵: #block[
为了将游戏世界的顶点放到屏幕上，每个矩阵的 3D 坐标首先被转换为相对于摄像机的坐标。这是通过\_视图矩阵\_完成的。其次，顶点通过\_投影矩阵\_投影到 2D 屏幕空间上：
#box(image("../docs/en/manuals/images/shader/projection.png"))
]
...
/ 常量: #block[
着色器常量在渲染绘制调用的持续时间内保持不变。常量被添加到材质文件的#emph[常量]部分，然后在着色器程序中声明为 `uniform`。采样器 uniform 被添加到材质的#emph[采样器]部分，然后在着色器程序中声明为 `uniform`。在顶点着色器中执行顶点变换所需的矩阵可作为常量使用：
- `CONSTANT_TYPE_WORLD` 是#emph[世界矩阵]，将对象从局部坐标空间映射到世界空间。
- `CONSTANT_TYPE_VIEW` 是#emph[视图矩阵]，将世界空间映射到摄像机空间。
- `CONSTANT_TYPE_PROJECTION` 是#emph[投影矩阵]，将摄像机空间映射到屏幕空间。
- 预乘的 $w o r l d * v i e w$、$v i e w * p r o j e c t i o n$ 和 $w o r l d * v i e w$ 矩阵也可用。
...
/ 采样器: #block[
着色器可以声明#emph[采样器]类型的 uniform 变量。采样器用于从图像源读取值：
...
您只能在 GLSL 标准库的纹理查找函数中使用采样器。材质手册解释了如何指定采样器设置。
]

/ UV 坐标: #block[
2D 坐标与顶点关联，它映射到 2D 纹理上的一个点。因此，纹理的一部分或整个部分可以绘制到由一组顶点描述的形状上。
#box(image("../docs/en/manuals/images/shader/uv_map.png"))
...
+ 在顶点着色器中为每个顶点设置 varying 变量。
+ 在光栅化期间，为正在渲染的图元上的每个片段插值此值。片段到形状顶点的距离决定了插值。
+ 该变量为每次对片段着色器的调用设置，可用于片段计算。

#box(image("../docs/en/manuals/images/shader/varying_vertex.png"))

例如，在三角形的每个角上将 varying 设置为 `vec3` RGB 颜色值将在整个形状上插值颜色。类似地，在矩形中的每个顶点上设置纹理贴图查找坐标（或#emph[UV 坐标]）允许片段着色器为形状的整个区域查找纹理颜色值。
#box(image("../docs/en/manuals/images/shader/varying.png"))
]
...
+ 传统管线，其中着色器使用与 ES2 兼容的 GLSL 代码编写。
+ 现代管线，其中着色器使用与 SPIR-v 兼容的 GLSL 代码编写。
...
=== 头文件保护
<头文件保护>
片段本身可以包含其他 `.glsl` 文件，这意味着最终生成的着色器可能多次包含相同的代码，根据文件的内容，您可能最终会因为多次声明相同的符号而导致编译问题。为了避免这种情况，您可以使用#emph[头文件保护]，这是几种编程语言中的常见概念。示例：
...
#box(image("../docs/en/manuals/images/shader/pipeline.png"))
...
#box(image("../docs/en/manuals/images/shader/vertex_shader.png"))
...
+ 包含视图和投影矩阵相乘的 uniform（常量）。
+ 精灵顶点的属性。`position` 已经被转换为世界空间。`texcoord0` 包含顶点的 UV 坐标。
+ 声明一个 varying 输出变量。这个变量将为每个片段在每个顶点设置的值之间进行插值，并发送到片段着色器。
+ `gl_Position` 被设置为投影空间中当前顶点的输出位置。这个值有 4 个组件：`x`、`y`、`z` 和 `w`。`w` 组件用于计算透视校正插值。在任何变换矩阵应用之前，这个值通常对于每个顶点都是 1.0。
+ 为这个顶点位置设置 varying UV 坐标。光栅化后，它将为每个片段进行插值，并发送到片段着色器。

顶点着色后，组件的屏幕形状已经确定：生成基本图元并进行光栅化，这意味着图形硬件将每个形状分割成#emph[片段]，或像素。然后它运行片段着色器程序，每个片段一次。对于屏幕上 16x24 像素大小的图像，程序运行 384 次，每次都是并行的。

#box(image("../docs/en/manuals/images/shader/fragment_shader.png"))

程序的输入是渲染管线和顶点着色器发送的任何内容，通常是片段的#emph[uv 坐标]、染色颜色等。输出是像素的最终颜色（`gl_FragColor`）。
...
+ 声明 varying 纹理坐标变量。这个变量将基于形状的顶点为每个片段进行插值。
+ 声明 `sampler2D` uniform 变量。采样器，连同插值的纹理坐标，用于精灵的纹理采样。对于精灵，引擎会自动将采样的纹理映射到精灵的#emph[图像]属性上。
+ 定义 `CONSTANT_TYPE_USER` 材质常量并声明为 `uniform`。用于为精灵设置染色颜色。默认值为纯白色。
+ 将染色颜色与其不透明度相乘，因为运行时纹理颜色都是经过不透明度预乘的。
+ 在插值坐标处采样并返回颜色值。
+ `gl_FragColor` 代表片段的最终颜色结果：漫反射纹理颜色与其染色颜色的乘积。

获得片段的最终颜色后，它还要经过一系列测试。常见的是#emph[深度测试]，看片段的深度值是否与像素深度缓冲区匹配。经过测试后，片段可能被丢弃或深度缓冲区被赋予新值。这个测试通常用于计算远离摄像机物体的渲染剔除工作。

如果这个片段被保留下来，它还要与早先进入帧缓冲区的像素数据进行#emph[混合]。渲染脚本基于混合参数将源颜色（片段着色器输出颜色）与目标颜色（帧缓冲区中已有颜色）进行混合计算。混合计算常见用法如显示半透明图像等。

== 深入学习
<深入学习>
- Shadertoy 上有大量开发者开源着色器。可以通过学习各种着色技术作为自己的灵感源泉。其中很多着色器改改就能应用到 Defold 中去。Shadertoy 教程 介绍了把网站着色器用于 Defold 的具体步骤。

- 渐变教程 介绍了使用纹理采样进行全屏颜色渐变效果的编写方法。

- The Book of Shaders 介紹了將著色器應用於項目的方法, 有利於提高性能和視覺效果。
