...
材质包含 #emph[标签]，这些信息在渲染管线中用于选择要渲染的对象。它还包含对 #emph[着色器程序] 的引用，这些程序通过可用的图形驱动程序编译，并上传到图形硬件，在组件每帧渲染时运行。

- 有关渲染管线的更多信息，请参阅渲染文档。
- 有关着色器程序的深入解释，请参阅着色器文档。

== 创建材质
<创建材质>
要创建材质，在 #emph[Assets] 浏览器中右键点击目标文件夹，然后选择 New… ▸ Material。（您也可以从菜单中选择 File ▸ New…，然后选择 Material）。为新材质文件命名，然后点击 Ok。

#box(image("../docs/en/manuals/images/materials/material_file.png"))

新材质将在 #emph[材质编辑器] 中打开。

#box(image("../docs/en/manuals/images/materials/material.png"))

材质文件包含以下信息：

/ Name: #block[
材质的标识符。此名称用于在 #emph[Render] 资源中列出材质，以将其包含在构建中。该名称也用于渲染 API 函数 `render.enable_material()`。名称应该是唯一的。
]

/ Vertex Program: #block[
使用材质渲染时要使用的顶点着色器程序文件（#emph[`.vp`]）。顶点着色器程序在 GPU 上运行，用于处理组件的每个原始顶点。它计算每个顶点的屏幕位置，并可选地输出”变化”变量，这些变量被插值并输入到片段程序。
]

/ Fragment Program: #block[
使用材质渲染时要使用的片段着色器程序文件（#emph[`.fp`]）。该程序在 GPU 上运行，用于处理每个原始片段（像素），其目的是决定每个片段的颜色。这通常通过纹理查找和基于输入变量（变化变量或常量）的计算来完成。
]

/ Vertex Constants: #block[
将传递给顶点着色器程序的统一变量。有关可用常量的列表，请参见下文。
]

/ Fragment Constants: #block[
...
/ Tags: #block[
与材质关联的标签。标签在引擎中表示为 #emph[位掩码]，由 `render.predicate()` 用于收集应该一起绘制的组件。有关如何执行此操作，请参阅渲染文档。您可以在项目中使用的最大标签数为 32。
]
...
/ Semantic type: #block[
语义类型指示属性的语义含义（#emph[是什么]）和/或#emph[如何在编辑器中显示]。例如，指定具有 `SEMANTIC_TYPE_COLOR` 的属性将在编辑器中显示颜色选择器，而数据仍将原样从引擎传递到着色器。
...
/ Value: #block[
属性的值。属性值可以基于每个组件被覆盖，但否则这将作为顶点属性的默认值。注意：对于#emph[默认]属性（位置、纹理坐标和页索引），该值将被忽略。
]
...
#box(image("../docs/en/manuals/images/materials/set_custom_attribute.png"))
...
#box(image("../docs/en/manuals/images/materials/instancing-setup.png"))

材质配置如下，具有一个每个实例重复的自定义顶点属性：

#box(image("../docs/en/manuals/images/materials/instancing-material.png"))
...
#box(image("../docs/en/manuals/images/materials/instancing-draw-calls.png"))
...
== 顶点和片段常量
<顶点和片段常量>
着色器常量，或称为”uniforms”，是从引擎传递到顶点和片段着色器程序的值。要使用常量，您在材质文件中将其定义为#emph[顶点常量]属性或#emph[片段常量]属性。需要在着色器程序中定义相应的 `uniform` 变量。材质中可以设置以下常量：

/ `CONSTANT_TYPE_WORLD`: #block[
世界矩阵。用于将顶点转换为世界空间。对于某些组件类型，由于批处理，顶点到达顶点程序时已经在世界空间中。在这些情况下，在着色器中与世界矩阵相乘将产生错误的结果。
]

/ `CONSTANT_TYPE_VIEW`: #block[
视图矩阵。用于将顶点转换为视图（相机）空间。
]

/ `CONSTANT_TYPE_PROJECTION`: #block[
投影矩阵。用于将顶点转换为屏幕空间。
]

/ `CONSTANT_TYPE_VIEWPROJ`: #block[
已经相乘的视图和投影矩阵。
]

/ `CONSTANT_TYPE_WORLDVIEW`: #block[
已经相乘的世界和视图矩阵。
]

/ `CONSTANT_TYPE_WORLDVIEWPROJ`: #block[
已经相乘的世界、视图和投影矩阵。
]

/ `CONSTANT_TYPE_NORMAL`: #block[
用于计算法线方向的矩阵。世界变换可能包含非均匀缩放，这会破坏组合世界视图变换的正交性。法线矩阵用于避免变换法线时的方向问题。（法线矩阵是世界视图矩阵的转置逆矩阵）。
]

/ `CONSTANT_TYPE_USER`: #block[
一个 vector4 常量，您可以用于任何想要传递到着色器程序的自定义数据。您可以在常量定义中设置常量的初始值，但它可以通过函数 go.set() / go.animate() 进行修改。您也可以使用 go.get() 检索值。更改单个组件实例的材质常量会破坏渲染批处理并导致额外的绘制调用。
]
...
/ `CONSTANT_TYPE_USER_MATRIX4`: #block[
一个 matrix4 常量，您可以用于任何想要传递到着色器程序的自定义数据。您可以在常量定义中设置常量的初始值，但它可以通过函数 go.set() / go.animate() 进行修改。您也可以使用 go.get() 检索值。更改单个组件实例的材质常量会破坏渲染批处理并导致额外的绘制调用。
]
...
精灵、瓦片地图、GUI 和粒子效果组件自动获得 `sampler2D` 集。着色器程序中第一个声明的 `sampler2D` 自动绑定到图形组件中引用的图像。因此，目前不需要为这些组件在材质文件中指定任何采样器。此外，这些组件类型目前仅支持单个纹理。（如果您需要在着色器中使用多个纹理，可以使用 `render.enable_texture()` 并从渲染脚本手动设置纹理采样器。）

#box(image("../docs/en/manuals/images/materials/sprite_sampler.png"))
...
您可以通过在材质文件中按名称添加采样器来指定组件的采样器设置。如果您没有在材质文件中设置采样器，将使用全局 #emph[graphics] 项目设置。

#box(image("../docs/en/manuals/images/materials/my_sampler.png"))

对于模型组件，您需要在材质文件中指定您想要的采样器设置。然后，编辑器将允许您为任何使用该材质的模型组件设置纹理：

#box(image("../docs/en/manuals/images/materials/model_samplers.png"))
...
#box(image("../docs/en/manuals/images/materials/model.png"))
