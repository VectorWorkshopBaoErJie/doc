= 项目设置
<项目设置>
文件 #emph[game.project] 包含所有项目级别的设置。它必须保存在项目的根文件夹中，并且必须命名为 #emph[game.project]。引擎在启动和运行您的游戏时做的第一件事就是查找这个文件。

文件中的每个设置都属于一个类别。当您打开文件时，Defold 会将所有设置按类别分组展示。

#box(image("../docs/en/manuals/images/project-settings/settings.jpg"))

== 文件格式
<文件格式>
#emph[game.project] 中的设置通常在 Defold 内部修改，但该文件也可以在任何标准文本编辑器中编辑。该文件遵循 INI 文件格式标准，如下所示：
...
这意味着设置 #emph[main\_collection] 属于 #emph[bootstrap] 类别。每当使用文件引用时（如上面的例子），路径需要附加一个 'c' 字符，这意味着您引用的是文件的编译版本。还要注意，包含 #emph[game.project] 的文件夹将成为项目根目录，这就是为什么在设置路径中有一个初始 '/'。

== 运行时访问
<运行时访问>
可以在运行时使用 `sys.get_config_string(key)`、`sys.get_config_number(key)` 和 `sys.get_config_int(key)` 从 #emph[game.project] 中读取任何值。例子：
...
==== Write Log
<write-log>
勾选时，引擎将写入日志文件。如果从编辑器运行多个实例，文件将被命名为 #emph[instance\_2\_log.txt]，其中 `2` 是实例索引。如果运行单个实例或从打包运行，文件将被命名为 #emph[log.txt]。日志文件的位置将是以下路径之一（按顺序尝试）：

+ 在 #emph[project.log\_dir] 中指定的路径（隐藏设置）
+ 系统日志路径：
...
==== Dependencies
<dependencies>
项目 #emph[库 URL] 的 URL 列表。有关更多信息，请参阅库手册。
...
==== Include Dirs
<include-dirs>
应通过库共享从您的项目中共享的目录的空格分隔列表。有关更多信息，请参阅库手册。
...
==== High Dpi
<high-dpi>
在支持的显示器上创建高 dpi 后缓冲区。通常游戏将以比#emph[宽度]和#emph[高度]设置中设置的分辨率高一倍的分辨率渲染，这仍然是脚本和属性中使用的逻辑分辨率。

==== Samples
<samples>
用于超级采样抗锯齿的样本数量。它设置 GLFW\_FSAA\_SAMPLES 窗口提示。值为 `0` 表示关闭抗锯齿。
...
==== Update Frequency
<update-frequency>
所需的帧率（以赫兹为单位）。设置为 0 表示可变帧率。大于 0 的值将导致固定帧率，在运行时上限为实际帧率（这意味着您不能在引擎帧中更新游戏循环两次）。使用 `sys.set_update_frequency(hz)` 在运行时更改此值。此设置也适用于无头构建。

==== Swap interval
<swap-interval>
此整数值控制应用程序如何处理垂直同步。0 禁用垂直同步，默认值为 1。使用 OpenGL 适配器时，此值设置窗口应在缓冲区交换之间更新的帧数。对于 Vulkan，没有内置的交换间隔概念，该值控制是否应启用垂直同步。

==== Vsync
<vsync>
依赖硬件垂直同步进行帧时序。可以根据图形驱动程序和平台特性进行覆盖。对于已弃用的 'variable\_dt' 行为，请取消勾选此设置并将帧上限设置为 0。

==== Display Profiles
<display-profiles>
指定要使用的显示配置文件，默认为 `/builtins/render/default.display_profilesc`。在GUI 布局手册中了解更多信息。
...
==== Max Instances
<max-instances>
集合中游戏对象实例的最大数量，默认为 `1024`。(参见组件最大数量优化的信息)。
...
==== Stream Cache Size
<stream-cache-size>
包含\_所有\_块的声音块缓存的最大大小。默认为 `2097152` 字节。
...
==== Max Count
<max-count-8>
集合代理的最大数量。(有关组件最大数量优化的信息)。

=== Collection factory
<collection-factory>
==== Max Count
<max-count-9>
集合工厂的最大数量。(有关组件最大数量优化的信息)。

=== Factory
<factory>
==== Max Count
<max-count-10>
游戏对象工厂的最大数量。(有关组件最大数量优化的信息)。

=== iOS
<ios>
==== App Icon 57x57--180x180
<app-icon-57x57180x180>
用作给定宽度和高度尺寸 `W` × `H` 的应用图标的图像文件 (.png)。

==== Launch Screen
<launch-screen>
Storyboard 文件 (.storyboard)。了解如何在 iOS 手册 中创建一个。
...
==== Bundle Name
<bundle-name>
捆绑短名称（15 个字符）（参见 `CFBundleName`）。

==== Bundle Version
<bundle-version>
捆绑版本，可以是数字或 x.y.z（参见 `CFBundleVersion`）。

==== Info.plist
<info.plist>
如果指定，在打包应用时使用此 #emph[`info.plist`] 文件。
...
==== Default Language
<default-language>
如果应用在 `Localizations` 列表中没有用户的首选语言时使用的语言（参见 `CFBundleDevelopmentRegion`）。如果首选语言在那里可用，请使用两字母 ISO 639-1 标准或三字母 ISO 639-2。

==== Localizations
<localizations>
此字段包含逗号分隔的字符串，标识支持本地化的语言名称或 ISO 语言指示符（参见 `CFBundleLocalizations`）。

=== Android
<android>
==== App Icon 36x36--192x192
<app-icon-36x36192x192>
用作给定宽度和高度尺寸 `W` × `H` 的应用图标的图像文件 (.png)。

==== Push Icon Small--LargeXxxhdpi
<push-icon-smalllargexxxhdpi>
...
==== Iap Provider
<iap-provider>
指定要使用的商店。有效选项是 `Amazon` 和 `GooglePlay`。有关更多信息，请参考 extension-iap。
...
==== Debuggable
<debuggable>
应用是否可以使用诸如 GAPID 或 Android Studio 之类的工具进行调试。这将在 Android 清单中设置 `android:debuggable` 标志（官方文档）。
...
<extract-native-libraries>
指定包安装程序是否将原生库从 APK 提取到文件系统。如果设置为 `false`，您的原生库将以未压缩的形式存储在 APK 中。虽然您的 APK 可能更大，但您的应用程序加载速度更快，因为库在运行时直接从 APK 加载。这将在 Android 清单中设置 `android:extractNativeLibs` 标志（官方文档）。
...
==== App Icon
<app-icon-1>
用作 Windows 应用图标的图像文件 (.ico)。有关如何创建 .ico 文件的更多信息，请参阅 Windows 手册。

=== HTML5
<html5>
有关这些选项的更多信息，请参阅 HTML5 平台手册。
...
==== #emph[App Manifest]
<app-manifest>
如果设置，使用应用清单自定义引擎构建。这允许您从引擎中删除未使用的部分以减少最终二进制文件的大小。了解如何在 应用清单手册 中排除未使用的功能。
...
== 在引擎启动时设置配置值
<在引擎启动时设置配置值>
当引擎启动时，可以从命令行提供配置值，以覆盖 #emph[game.project] 设置：
...
自定义值可以像任何其他配置值一样，使用 `sys.get_config_string()` 或 `sys.get_config_number()` 读取：
...
== 自定义项目设置
<自定义项目设置>
可以为主项目或为 原生扩展 定义自定义设置。主项目的自定义设置必须在项目根目录的 `game.properties` 文件中定义。对于原生扩展，它们应该在 `ext.manifest` 文件旁边的 `ext.properties` 文件中定义。

设置文件使用与 #emph[game.project] 相同的 INI 格式，属性属性使用带后缀的点符号定义：
...
目前，元属性仅在 `bob.jar` 打包应用程序时使用，但稍后将被编辑器解析并在 #emph[game.project] 查看器中表示。
