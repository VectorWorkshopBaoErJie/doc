...
== 引入 Lua 文件
<引入-lua-文件>
存储在项目结构中某处以”.lua”为文件扩展名的 Lua 代码可以被引入到脚本和 GUI 脚本文件中. 要创建新的 Lua 模块文件，请在 #emph[Assets] 视图中右键单击您想要创建它的文件夹，然后选择 New… ▸ Lua Module. 给文件一个唯一的名称并按 Ok:

#box(image("../docs/en/manuals/images/modules/new_name.png"))
...
== 模块
<模块>
为了封装数据和函数，Lua 使用 #emph[模块]。Lua 模块是一个常规的 Lua 表，用于包含函数和数据。该表被声明为局部变量，以避免污染全局作用域：
...
首先，在 "module.lua" 中创建的表是在局部作用域中创建的，并且该表的 #emph[引用] 被返回给用户。重新加载 "module.lua" 会再次评估模块代码，但这会在局部作用域中创建一个新表，而不是更新 `m` 所引用的表。

其次，Lua 缓存所需的文件。当文件第一次被需要时，它被放在 `package.loaded` 表中，以便在后续需要时可以更快地读取。您可以通过将文件的条目设置为 nil 来强制文件从磁盘重新读取：`package.loaded["my_module"] = nil`。

要正确地热重载模块，您需要重新加载模块，重置缓存，然后重新加载所有使用该模块的文件。这远非最佳选择。

相反，您可以考虑在 #emph[开发期间] 使用的解决方法：将模块表放在全局作用域中，并让 `M` 引用全局表，而不是在每次文件评估时创建一个新表。重新加载模块会更改全局表的内容：
