...
== Lua 版本
<lua-版本>
Defold 使用 LuaJIT，这是一个高度优化的 Lua 版本，适用于游戏和其他性能关键软件。它与 Lua 5.1 完全向上兼容，并支持所有标准 Lua 库函数和完整的 Lua/C API 函数集。

LuaJIT 还添加了几个语言扩展和一些 Lua 5.2 和 5.3 的功能。
...
=== 标准库和扩展
<标准库和扩展>
Defold 包含所有 Lua 5.1 标准库 以及一个 socket 和位操作库：
...
所有库都在 参考 API 文档 中有文档说明。

== Lua 书籍和资源
<lua-书籍和资源>
=== 在线资源
<在线资源>
- Programming in Lua (第一版) 后续版本有印刷版。
- Lua 5.1 参考手册
- 15 分钟学习 Lua
- Awesome Lua - 教程部分
...
=== 书籍
<书籍>
- Programming in Lua - Programming in Lua 是关于该语言的官方书籍，为任何想要使用 Lua 的程序员提供了坚实的基础。作者是该语言的首席架构师 Roberto Ierusalimschy。
- Lua programming gems - 这本文集记录了一些关于如何用 Lua 编写好程序的现有智慧和经验。
- Lua 5.1 参考手册 - 也可以在线获取（见上文）
...
/ number: #block[
数字在内部表示为 64 位 #emph[整数] 或 64 位 #emph[浮点数]。Lua 根据需要自动在这些表示之间转换，因此您通常不必担心它。
```lua
print(10) --> 输出 '10'
print(10.0) --> '10'
...
#figure(
  align(center)[#table(
    ...
    [`\[`], [左方括号],
    [`\]`], [右方括号],
    [`\ddd`], [由其数值表示的字符，其中 `ddd` 是最多三个 #emph[十进制] 数字的序列],
  )]
  , kind: table
  )
...
/ table: #block[
表是 Lua 中唯一的数据结构类型。它们是关联数组 #emph[对象]，用于表示列表、数组、序列、符号表、集合、记录、图形、树等。表总是匿名的，分配给表的变量不包含表本身，而是对它的引用。当将表初始化为序列时，第一个索引是 `1`，而不是 `0`。
```lua
-- 初始化表中序列
...
/ userdata: #block[
提供用户数据以允许任意 C 数据存储在 Lua 变量中。Defold 使用 Lua userdata 对象来存储哈希值 (hash)、URL 对象 (url)、数学对象 (vector3, vector4, matrix4, quaternion)、游戏对象、GUI 节点 (node)、渲染谓词 (predicate)、渲染目标 (render\_target) 和渲染常量缓冲区 (constant\_buffer)
]
...
/ for: #block[
Lua 有两种类型的 `for` 循环：数值型和通用型。数值型 `for` 接受 2 或 3 个数值，而通用型 `for` 迭代由 #emph[迭代器] 函数返回的所有值。
```lua
-- 输出数字 1 到 10
for i = 1, 10 do
    print(i)
...
每个 Lua 源文件定义一个单独的作用域。在文件最顶层声明的局部变量意味着该变量对于 Lua 脚本文件是局部的。每个函数创建另一个嵌套作用域，每个控制结构块创建额外的作用域。您可以使用 `do` 和 `end` 关键字明确创建作用域。Lua 是词法作用域的，意味着作用域可以完全访问来自封闭作用域的 #emph[局部] 变量。请注意，局部变量必须在使用前声明。
...
== Defold 中的 Lua 上下文
<defold-中的-lua-上下文>
您声明的所有变量默认都是全局的，意味着它们在 Lua 运行时上下文的所有部分都可用。Defold 在 #emph[game.project] 中有一个 #emph[shared\_state] 设置来控制此上下文。如果设置了该选项，所有脚本、GUI 脚本和渲染脚本都在同一个 Lua 上下文中评估，全局变量在任何地方都可见。如果未设置该选项，引擎会在单独的上下文中执行脚本、GUI 脚本和渲染脚本。

#box(image("../docs/en/manuals/images/lua/lua_contexts.png"))
...
- 每个 #emph[新的唯一] 字符串都会创建一个新对象。编写 `local s = "some_string"` 将创建一个新对象并将 `s` 分配给它。局部变量 `s` 本身不会产生垃圾，但字符串对象会。多次使用相同的字符串不会增加额外的内存成本。
- 每次执行表构造函数（`{ ... }`）时都会创建一个新表。
- 执行 #emph[函数语句] 会创建一个闭包对象。（即执行语句 `function () ... end`，而不是调用已定义的函数）
- 可变参数函数（`function(v, ...) end`）在每次 #emph[调用] 函数时都会为省略号创建一个表（在 Lua 5.2 版本之前，或者如果不使用 LuaJIT）。
- `dofile()` 和 `dostring()`
- 用户数据对象
...
所以一个 `vector3` 的重量是 70-22=48 字节。这并不多，但如果您在 60 FPS 的游戏中每帧创建 #emph[一个]，突然间每秒就会产生 2.8 kB 的垃圾。如果有 360 个脚本组件，每个每帧创建一个 `vector3`，我们每秒将看到 1 MB 的垃圾生成。数字可以非常迅速地累加。当 Lua 运行时收集垃圾时，它可能会消耗掉许多宝贵的毫秒------尤其是在移动平台上。
...
默认的垃圾收集方案可能对某些时间关键的应用程序不是最优的。如果您在游戏或应用程序中看到卡顿，您可能希望通过 `collectgarbage()` Lua 函数调整 Lua 收集垃圾的方式。例如，您可以使用较低的 `step` 值每帧运行收集器一小段时间。要了解您的游戏或应用程序消耗了多少内存，您可以使用以下代码打印当前的垃圾字节数：
...
/ 模块中的行为: #block[
将行为封装在模块中允许您在不同的游戏对象脚本组件（和 GUI 脚本）之间轻松共享代码。在编写模块函数时，通常最好编写严格的函数式代码。在某些情况下，存储状态或副作用是必要的（或导致更清晰的设计）。如果必须在模块中存储内部状态，请注意组件共享 Lua 上下文。有关详细信息，请参阅模块文档。
#box(image("../docs/en/manuals/images/lua/lua_module.png"))
...
#box(image("../docs/en/manuals/images/lua/lua_helper.png"))
]

/ 在集合内将游戏对象与辅助行为对象分组: #block[
在此设计中，您可以创建一个自动作用于另一个目标游戏对象的行为游戏对象，要么通过预定义的名称（用户必须重命名目标游戏对象以匹配），要么通过指向目标游戏对象的 `go.property()` URL。
#box(image("../docs/en/manuals/images/lua/lua_collection.png"))
