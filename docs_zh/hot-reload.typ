...
#box(image("../docs/en/manuals/images/hot-reload/menu.png"))

== 在设备上热重载
<在设备上热重载>
热重载在设备上和桌面上一样有效。要在设备上使用它，请在您的移动设备上运行游戏的调试版本，或运行开发应用，然后在编辑器中选择它作为目标：

#box(image("../docs/en/manuals/images/hot-reload/target.png"))

现在当您构建并运行时，编辑器会将所有资源上传到设备上运行的应用程序并启动游戏。从那时起，您热重载的任何文件都将在设备上更新。

例如，要在手机上运行的游戏中显示的GUI上添加几个按钮，只需打开GUI文件：

#box(image("../docs/en/manuals/images/hot-reload/gui.png"))

添加新按钮，保存并热重载GUI文件。您现在可以在手机屏幕上看到新按钮：

#box(image("../docs/en/manuals/images/hot-reload/gui-reloaded.png"))
...
更改并重新加载”my\_module.lua”将\_不会\_改变”user.script”的行为。有关原因以及如何避免此陷阱的更多信息，请参见模块手册。
