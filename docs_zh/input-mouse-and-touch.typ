#block[
建议您熟悉Defold中输入的一般工作方式，如何接收输入以及在脚本文件中接收输入的顺序。有关输入系统的更多信息，请参阅输入系统概述手册。
...
#box(image("../docs/en/manuals/images/input/mouse_bindings.png"))
...
#box(image("../docs/en/manuals/images/input/touch_bindings.png"))

== 单点触摸
<单点触摸>
单点触摸类型触发器不是从输入绑定的触摸触发器部分设置的。相反，#strong[当您为`MOUSE_BUTTON_LEFT`或`MOUSE_BUTTON_1`设置了鼠标按钮输入时，单点触摸触发器会自动设置]。
...
#block[
Defold-Input资源可用于轻松设置虚拟屏幕控件，如按钮和支持多点触摸的模拟摇杆。
...
=== 检测与GUI节点的交互
<检测与gui节点的交互>
对于UI元素，有一个`gui.pick_node(node, x, y)`函数，它将根据指定坐标是否在GUI节点的边界内返回true或false。请参阅API文档、指针悬停示例或按钮示例了解更多信息。

=== 检测与游戏对象的交互
<检测与游戏对象的交互>
对于游戏对象，检测交互更加复杂，因为诸如摄像机平移和渲染脚本投影等因素将影响所需的计算。检测与游戏对象的交互有两种通用方法：

+ 跟踪用户可以与之交互的游戏对象的位置和大小，并检查鼠标或触摸坐标是否在任何对象的边界内。
+ 将碰撞对象附加到用户可以与之交互的游戏对象上，并附加一个跟随鼠标或手指的碰撞对象，并检查它们之间的碰撞。

#block[
可以在Defold-Input资源中找到使用碰撞对象检测用户输入的现成解决方案，支持拖动和点击。

]
在这两种情况下，都需要从鼠标或触摸事件的屏幕空间坐标和游戏对象的世界空间坐标进行转换。这可以通过几种不同的方式完成：

- 手动跟踪渲染脚本使用的视图和投影，并使用它在世界空间之间进行转换。有关此示例，请参阅摄像机手册。
- 使用第三方摄像机解决方案并利用提供的屏幕到世界转换函数。
