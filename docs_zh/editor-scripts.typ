...
== 编辑器脚本运行时
<编辑器脚本运行时>
编辑器脚本在编辑器内部运行，在 Java 虚拟机模拟的 Lua 虚拟机中运行。所有脚本共享同一个环境，这意味着它们可以相互交互。您可以导入 Lua 模块，就像使用 `.script` 文件一样，但编辑器内部运行的 Lua 版本不同，因此请确保您的共享代码是兼容的。编辑器使用 Lua 版本 5.2.x，更具体地说是 luaj 运行时，这目前是在 JVM 上运行 Lua 的唯一可行解决方案。除此之外，还有一些限制：
\- 没有 `debug` 包；
...
\- 在编辑器需要脚本立即响应的上下文中，不允许使用一些长时间运行的函数，详见执行模式。

所有在编辑器脚本中定义的编辑器扩展在您打开项目时都会加载。当您获取库时，扩展会重新加载，因为您依赖的库中可能有新的编辑器脚本。在此重新加载期间，不会获取您自己的编辑器脚本的更改，因为您可能正在更改它们。要同时重新加载它们，您应该运行 #strong[Project → Reload Editor Scripts] 命令。
...
然后编辑器会收集项目和库中定义的所有编辑器脚本，将它们加载到单个 Lua 虚拟机中，并在需要时调用它们（更多内容请参见命令和生命周期钩子部分）。
...
\- `"path"` --- #emph[资源] 的项目文件夹文件路径 --- 作为文件或目录存在的实体。返回值示例：`"/main/game.script"`
...
\- `editor.ui.*` --- 各种与 UI 相关的函数，请参见UI 手册。
\- `editor.prefs.*` --- 与编辑器首选项交互的函数，请参见首选项。

您可以在此处找到完整的编辑器 API 参考。
...
- `locations`（必需）--- 一个数组，包含 `"Edit"`、`"View"`、`"Project"`、`"Debug"`、`"Assets"`、`"Bundle"`、`"Scene"` 或 `"Outline"` 中的一个或多个，描述了该命令应该可用的位置。`"Edit"`、`"View"`、`"Project"` 和 `"Debug"` 表示顶部的菜单栏，`"Assets"` 表示资源窗格中的上下文菜单，`"Outline"` 表示大纲窗格中的上下文菜单，`"Bundle"` 表示 #strong[Project → Bundle] 子菜单。
...
=== 使用 shell 命令
<使用-shell-命令>
在`run`处理程序中，您可以写入文件（使用`io`模块）并执行shell命令（使用`editor.execute()`命令）。执行shell命令时，可以将shell命令的输出捕获为字符串，然后在代码中使用它。例如，如果您想创建一个格式化JSON的命令，该命令使用全局安装的`jq`，您可以编写以下命令：
...
#block[
如果您想将编辑器脚本作为库分发，您可能希望将编辑器平台的二进制程序捆绑在依赖项中。有关如何执行此操作的更多详细信息，请参阅库中的编辑器脚本。

]
== 生命周期钩子
<生命周期钩子>
有一个特殊的编辑器脚本文件: `hooks.editor_script`, 位于项目根目录, 就是跟 #emph[game.project] 并存于同一目录. 只有这个编辑器脚本会从编辑器获得生命周期事件. 脚本文件举例:
...
\- `platform` --- 打包的目标平台. 支持的平台值详见 Bob 教程.
...
== 语言服务器
<语言服务器>
编辑器支持 Language Server Protocol 的子集。虽然我们旨在未来扩展编辑器对 LSP 功能的支持，但目前它只能在编辑的文件中显示诊断（即 lints）并提供补全。
...
语言服务器定义表可以指定：
\- `languages`（必需）--- 服务器感兴趣的语言列表，如此处所定义（文件扩展名也可以工作）；
\- `command`（必需）- 命令及其参数的数组
\- `watched_files` - 带有 `pattern` 键（glob）的表数组，将触发服务器的监视文件更改通知。
...
== 执行模式
<执行模式>
编辑器脚本运行时使用两种执行模式，这些模式对编辑器脚本基本上是透明的：#strong[即时]和#strong[长时间运行]。

#strong[即时]模式用于编辑器需要尽快从脚本接收响应的情况。例如，菜单命令的 `active` 回调在即时模式下执行，因为这些检查是在编辑器 UI 线程上响应与编辑器的用户交互而执行的，并且应该在同一帧内更新 UI。

#strong[长时间运行]模式用于编辑器不需要脚本即时响应的情况。例如，菜单命令的 `run` 回调在#strong[长时间运行]模式下执行，允许脚本有更多时间来完成其工作。
...
现在编辑器以非阻塞方式与 Lua VM 交互，因此不再需要这些操作：使用像 `editor.execute()` 这样的函数更方便、简洁和强大。这些操作现在已#strong[弃用]，尽管我们没有计划删除它们。
...
`lua   {     action = "set",     node_id = opts.selection,     property = "text",     value = "current time is " .. os.date()   }`
`"set"` 操作需要以下键：
...
`lua   {     action = "shell",     command = {       "./scripts/minify-json.sh",       editor.get(opts.selection, "path"):sub(2) -- trim leading "/"     }   }`
