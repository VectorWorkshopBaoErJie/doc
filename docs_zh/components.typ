...
- Collection factory - 生成集合
- Collection proxy - 加载和卸载集合
- Collision object - 2D 和 3D 物理
- Camera - 更改游戏世界的视口和投影
- Factory - 生成游戏对象
- GUI - 渲染图形用户界面
- Label - 渲染一段文本
- Mesh - 显示3D网格（具有运行时创建和操作功能）
- Model - 显示3D模型（带有可选动画）
- Particle FX - 生成粒子
- Script - 添加游戏逻辑
- Sound - 播放声音或音乐
- Sprite - 显示2D图像（带有可选翻页动画）
- Tilemap - 显示瓦片网格

可以通过扩展添加其他组件：

- Rive model - 渲染Rive动画
- Spine model - 渲染Spine动画

== 启用和禁用组件
<启用和禁用组件>
游戏对象的组件在游戏对象创建时被启用。如果您希望禁用组件，可以通过向组件发送`disable`消息来完成：
...
=== 渲染脚本谓词
<渲染脚本谓词>
每个组件都被分配一个材质，每个材质有一个或多个标签。渲染脚本反过来定义多个谓词，每个谓词匹配一个或多个材质标签。渲染脚本谓词在渲染脚本的#emph[update()函数中逐个绘制]，匹配每个谓词中定义的标签的组件将被绘制。默认渲染脚本首先在一个通道中绘制精灵和瓦片地图，然后在另一个通道中绘制粒子效果，两者都在世界空间中。然后渲染脚本将继续在屏幕空间中的单独通道中绘制GUI组件。
...
#box(image("../docs/en/manuals/images/graphics/z-order.png"))

匹配渲染谓词的组件一起绘制，它们绘制的顺序取决于组件的最终z值。组件的最终z值是组件本身的z值、它所属的游戏对象的z值以及任何父游戏对象的z值之和。

#block[
多个GUI组件的绘制顺序#strong[不是]由GUI组件的z值决定的。GUI组件绘制顺序由gui.set\_render\_order()函数控制。
...
#box(image("../docs/en/manuals/images/graphics/component-hierarchy.png"))
