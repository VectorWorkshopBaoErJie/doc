...
#box(image("../docs/en/manuals/images/live-update/mona-lisa.png"))

要让引擎动态加载这样的集合，我们可以简单地添加一个集合代理组件并将其指向 #emph[`monalisa.collection`]。现在游戏可以通过向集合代理发送 `load` 消息来选择何时将集合中的内容从存储加载到内存中。然而，我们希望更进一步，自己控制集合中包含的资源的加载。

这只需在集合代理属性中勾选 #emph[排除] 复选框，告诉 Defold 在创建应用程序包时将 #emph[`monalisa.collection`] 中的任何内容排除在外。

#block[
基础游戏包引用的任何资源都不会被排除。

]
#box(image("../docs/en/manuals/images/live-update/proxy-excluded.png"))

== Live Update 设置
<live-update-设置>
当 Defold 创建应用程序包时，它需要将任何排除的资源存储在某处。Live Update 的项目设置控制这些资源的位置。这些设置可以在 Project ▸ Live update Settings… 下找到。这将创建一个设置文件（如果尚不存在）。在 #emph[game.project] 中，选择打包时使用的 live-update 设置文件。这允许为不同的环境使用不同的 live-update 设置，例如生产环境、QA、开发环境等。

#box(image("../docs/en/manuals/images/live-update/05-liveupdate-settings-zip.png"))

目前 Defold 有两种方式可以存储资源。在设置窗口的 #emph[模式] 下拉菜单中选择方法：

/ `Zip`: #block[
此选项告诉 Defold 创建一个包含任何排除资源的 Zip 归档文件。归档文件保存在 #emph[导出路径] 设置中指定的位置。
]
...
/ `Amazon`: #block[
此选项告诉 Defold 自动将排除的资源上传到 Amazon Web Service (AWS) S3 存储桶。填写您的 AWS #emph[凭证配置文件] 名称，选择适当的 #emph[存储桶] 并提供 #emph[前缀] 名称。您可以在此 aws 指南 中阅读有关如何设置 AWS 账户的更多信息。
]
...
#box(image("../docs/en/manuals/images/live-update/bundle-app.png"))

打包时，任何排除的资源都将被排除在应用程序包之外。通过勾选 #emph[发布 Live update 内容] 复选框，您告诉 Defold 根据您设置 Live Update 设置的方式（见上文）将排除的资源上传到 Amazon 或创建 Zip 归档文件。包的清单文件也将包含在排除的资源中。

点击 #emph[打包] 并选择应用程序包的位置。现在您可以启动应用程序并检查一切是否按预期工作。
...
我们建议为每个挂载点存储一些元数据，以便 #emph[在启动时]，开发者可以决定是否应该删除挂载点/归档文件。
一种方法是在游戏打包后向 zip 归档文件添加一个额外的文件。例如，通过插入一个 `metadata.json`，其中包含游戏所需的任何相关信息。然后，在启动时，游戏可以使用 `sys.load_resource("/metadata.json")` 检索它。#emph[请注意，您需要为每个挂载点的自定义数据使用唯一的名称，否则挂载点将为您提供具有最高优先级的文件]
...
阅读更多关于如何使用 live update 进行脚本编写的内容。

== 开发注意事项
<开发注意事项>
/ 调试: #block[
当运行游戏的打包版本时，您没有直接访问控制台的权限。这会导致调试问题。但是，您可以从命令行运行应用程序，或者通过双击包中的可执行文件直接运行：
#box(image("../docs/en/manuals/images/live-update/run-bundle.png"))

现在游戏启动时带有一个 shell 窗口，将输出任何 `print()` 语句：
#box(image("../docs/en/manuals/images/live-update/run-bundle-console.png"))
]
...
#box(image("../docs/en/manuals/images/live-update/local-storage.png"))
]
