= 消息传递
<消息传递>
消息传递是 Defold 游戏对象之间进行通信的机制。本手册假设您已对 Defold 的寻址机制和基本构建块有基本了解。
...
+ 一个主引导集合，包含一个带有 GUI 组件的游戏对象（GUI 由一个迷你地图和一个分数计数器组成）。还有一个 ID 为 "level" 的集合。
+ 名为 "level" 的集合包含两个游戏对象：一个英雄玩家角色和一个敌人。

#box(image("../docs/en/manuals/images/message_passing/message_passing_structure.png"))

#block[
本示例的内容存在于两个单独的文件中。一个用于主引导集合，另一个用于 ID 为 "level" 的集合。然而，在 Defold 中，文件名\_并不重要\_。重要的是您为实例分配的身份标识。

]
游戏包含一些需要对象间通信的简单机制：

#box(image("../docs/en/manuals/images/message_passing/message_passing.png"))
...
在这种情况下，代码只查看消息的名称（作为哈希字符串在参数 `message_id` 中发送）。代码不关心消息数据或发送者---#emph[任何人]发送 "punch" 消息都会对可怜的敌人造成伤害。
]
...
== 游戏世界之间的消息传递
<游戏世界之间的消息传递>
如果您使用集合代理将新游戏世界加载到运行时中，您将需要在游戏世界之间传递消息。假设您已通过代理加载了一个集合，并且该集合的#emph[名称]属性设置为 "level"：

#box(image("../docs/en/manuals/images/message_passing/collection_name.png"))
