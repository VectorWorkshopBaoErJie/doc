...
== 向GUI添加脚本
<向gui添加脚本>
要向GUI添加脚本，首先通过在#emph[Assets]浏览器中右键点击一个位置并从弹出上下文菜单中选择New ▸ Gui Script来创建GUI脚本文件。
...
要将脚本附加到GUI组件，请打开GUI组件原型文件（在其他引擎中也称为”prefabs”或”blueprints”），并在#emph[Outline]中选择根节点以调出GUI#emph[Properties]。将#emph[Script]属性设置为脚本文件。

#box(image("../docs/en/manuals/images/gui-script/set_script.png"))

如果GUI组件已添加到游戏中某处的游戏对象，脚本现在将运行。

== "gui"命名空间
<gui命名空间>
GUI脚本可以访问`gui`命名空间和所有gui函数。`go`命名空间不可用，因此您需要将游戏对象逻辑分离到脚本组件中，并在GUI和游戏对象脚本之间进行通信。任何尝试使用`go`函数的操作都会导致错误：
...
#box(image("../docs/en/manuals/images/gui-script/message_passing.png"))

== 寻址节点
<寻址节点>
GUI节点可以通过附加到组件的GUI脚本进行操作。每个节点必须在编辑器中设置唯一的#emph[Id]：

#box(image("../docs/en/manuals/images/gui-script/node_id.png"))

#emph[Id]允许脚本获取对节点的引用，并使用gui命名空间函数对其进行操作：
...
#box(image("../docs/en/manuals/images/gui-script/dynamic_nodes.png"))
