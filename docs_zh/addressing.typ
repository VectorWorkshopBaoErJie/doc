...
#box(image("../docs/en/manuals/images/addressing/bean_editor.png"))
...
+ 如果你对'\#'字符感到困惑，不用担心，我们很快会讲到它。

这将按预期工作。当游戏启动时，脚本组件#emph[通过其标识符”body”寻址]到精灵组件，并使用该地址向其发送一条带有”disable”的#emph[消息]。这个特殊引擎消息的效果是精灵组件隐藏精灵图形。从示意图上看，设置如下：

#box(image("../docs/en/manuals/images/addressing/bean.png"))

设置中的标识符是任意的。在这里，我们选择给游戏对象命名为”bean”，其精灵组件命名为”body”，而控制角色的脚本组件命名为”controller”。

#block[
如果你不选择名称，编辑器会为你选择。每当你在编辑器中创建新的游戏对象或组件时，会自动设置一个唯一的#emph[Id]属性。
...
#box(image("../docs/en/manuals/images/addressing/bean_shield_editor.png"))

新组件必须在游戏对象内唯一标识。如果你给它命名为”body”，脚本代码将不清楚应该向哪个精灵发送”disable”消息。因此我们选择了唯一（且描述性）的标识符”shield”。现在我们可以随意启用和禁用”body”和”shield”精灵。

#box(image("../docs/en/manuals/images/addressing/bean_shield.png"))

#block[
如果你确实尝试多次使用同一标识符，编辑器会发出错误信号，因此在实践中这永远不会成为问题：

#box(image("../docs/en/manuals/images/addressing/name_collision.png"))
...
#box(image("../docs/en/manuals/images/addressing/bean_buddy.png"))
...
回到前面只有一个游戏对象的例子，我们看到通过省略目标地址的游戏对象标识符部分，代码可以寻址#emph[当前游戏对象]中的组件。

例如，`"#body"`表示当前游戏对象中组件”body”的地址。这非常有用，因为这段代码可以在#emph[任何]游戏对象中工作，只要存在”body”组件。

== 集合
<集合>
集合使得创建游戏对象的组或层次结构，并以受控方式重用它们成为可能。当你在游戏中填充内容时，可以在编辑器中使用集合文件作为模板（或”原型”或”预制件”）。

假设你想要创建大量的bean/buddy团队。一个好的方法是在一个新的#emph[集合文件]中创建一个模板（命名为”team.collection”）。在集合文件中构建团队游戏对象并保存它。然后将该集合文件内容的实例放入你的主引导集合中，并给该实例一个标识符（命名为”team\_1”）：

#box(image("../docs/en/manuals/images/addressing/team_editor.png"))

有了这种结构，"bean"游戏对象仍然可以通过地址`"buddy#controller"`引用”buddy”中的”controller”组件。

#box(image("../docs/en/manuals/images/addressing/collection_team.png"))

如果你添加”team.collection”的第二个实例（命名为”team\_2”），在”team\_2”脚本组件中运行的代码将同样工作良好。来自集合”team\_2”的”bean”游戏对象实例仍然可以通过地址`"buddy#controller"`寻址”buddy”中的”controller”组件。

#box(image("../docs/en/manuals/images/addressing/teams_editor.png"))

== 相对寻址
<相对寻址>
地址`"buddy#controller"`对两个集合中的游戏对象都有效，因为它是一个#emph[相对]地址。每个集合”team\_1”和”team\_2”都创建了一个新的命名上下文，或者如果你愿意，可以称为”命名空间”。Defold通过考虑集合创建的命名上下文来避免命名冲突：

#box(image("../docs/en/manuals/images/addressing/relative_same.png"))

- 在命名上下文”team\_1”中，游戏对象”bean”和”buddy”被唯一标识。
- 类似地，在命名上下文”team\_2”中，游戏对象”bean”和”buddy”也被唯一标识。
...
+ 编辑器读取引导集合（"main.collection"）及其所有内容（游戏对象和其他集合）。
+ 对于每个静态游戏对象，编译器创建一个标识符。这些被构建为从引导根开始，沿着集合层次结构到对象的”路径”。每层都添加一个'/'字符。
...
#box(image("../docs/en/manuals/images/addressing/manager_editor.png"))

然后每个豆子负责向管理器发送状态消息：如果发现敌人则发送”contact”，如果被击中并受到伤害则发送”ouch!"。为了使这个工作，豆子控制器脚本使用绝对寻址向"manager”中的”controller”组件发送消息。

任何以'/'开头的地址将从游戏世界的根解析。这对应于游戏启动时加载的#emph[引导集合]的根。

管理器脚本的绝对地址是`"/manager#controller"`，这个绝对地址将解析到正确的组件，无论它在哪里使用。

#box(image("../docs/en/manuals/images/addressing/teams_manager.png"))

#box(image("../docs/en/manuals/images/addressing/absolute.png"))
...
/ socket: #block[
标识目标的游戏世界。在使用集合代理时这很重要，然后用于标识\_动态加载的集合\_。
]
