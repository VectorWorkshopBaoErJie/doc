...
/ 声音组: #block[
每个声音组件可以被指定属于一个 #emph[组]。组提供了一种直观的方式来管理属于一起的声音。例如，可以设置一个 "sound\_fx" 组，任何属于该组的声音都可以通过简单的函数调用进行减弱。
]

== 创建声音组件
<创建声音组件>
声音组件只能在游戏对象中原地实例化。创建一个新的游戏对象，右键单击它并选择 Add Component ▸ Sound，然后按 #emph[OK]。

#box(image("../docs/en/manuals/images/sound/sound_add_component.jpg"))

创建的组件有一组应该设置的属性：

#box(image("../docs/en/manuals/images/sound/sound_properties.png"))

/ #emph[Sound]: #block[
应该设置为项目中的声音文件。文件应该是 #emph[Wave]、#emph[Ogg Vorbis] 或 #emph[Ogg Opus] 格式。Defold 支持以 16bit 位深保存的声音文件。
]

/ #emph[Looping]: #block[
如果选中，声音将播放 #emph[Loopcount] 次或直到明确停止。
]

/ #emph[Loopcount]: #block[
...
== 播放声音
<播放声音>
当您正确设置了声音组件后，可以通过调用 `sound.play()` 使其播放声音：
...
#block[
即使声音组件所属的游戏对象被删除，声音也会继续播放。您可以调用 `sound.stop()` 来停止声音（见下文）。
...
== 增益
<增益>
#box(image("../docs/en/manuals/images/sound/sound_gain.png"))

声音系统有 4 级增益：

- 在声音组件上设置的增益。
- 在通过调用 `sound.play()` 启动声音时或通过调用 `sound.set_gain()` 更改声音上的增益时设置的增益。
- 通过 `sound.set_group_gain()` 函数调用在组上设置的增益。
...
有一些函数可用于获取所有可用的组，获取字符串名称，获取和设置增益、均方根（参见 http:\/\/en.wikipedia.org/wiki/Root\_mean\_square）和峰值增益。还有一个函数允许您测试目标设备的音乐播放器是否正在运行：
...
组通过哈希值标识。字符串名称可以通过 `sound.get_group_name()` 检索，可用于在开发工具中显示组名称，例如用于测试组电平的混音器。

#box(image("../docs/en/manuals/images/sound/sound_mixer.png"))
...
== 声音门控
<声音门控>
如果您的游戏在某个事件上播放相同的声音，并且该事件经常被触发，您可能会冒着几乎同时播放相同声音两次或更多次的风险。如果发生这种情况，声音将 #emph[相位偏移]，这可能导致一些非常明显的伪影。

#box(image("../docs/en/manuals/images/sound/sound_phase_shift.png"))
...
== 运行时操作
<运行时操作>
您可以通过许多不同的属性在运行时操作声音（有关用法，请参阅 API 文档）。可以使用 `go.get()` 和 `go.set()` 操作以下属性：
...
== 项目配置
<项目配置>
#emph[game.project] 文件有一些与声音组件相关的项目设置。
