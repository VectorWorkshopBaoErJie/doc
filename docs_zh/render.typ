...
=== 渲染管线 - 是什么、何时以及何处？
<渲染管线---是什么何时以及何处>
渲染管线控制渲染什么、何时渲染以及在哪里渲染。渲染什么由渲染判定控制。何时渲染判定在渲染脚本中控制，在哪里渲染判定由视图投影控制。渲染管线还可以剔除位于定义边界框或视锥体之外的图形，这些图形由渲染判定绘制。这个过程称为视锥体剔除。

== 默认渲染
<默认渲染>
渲染文件包含对当前渲染脚本的引用，以及应在渲染脚本中可用的自定义材质（使用 `render.enable_material()`）

渲染管线的核心是\_渲染脚本\_。这是一个包含 `init()`、`update()` 和 `on_message()` 函数的 Lua 脚本，主要用于与底层图形 API 交互。渲染脚本在游戏生命周期中具有特殊位置。详细信息可在应用程序生命周期文档中找到。

在项目的”Builtins”文件夹中，您可以找到默认渲染资源（"default.render"）和默认渲染脚本（"default.render\_script"）。

#box(image("../docs/en/manuals/images/render/builtin.png"))

要设置自定义渲染器：

+ 将”default.render”和”default.render\_script”文件复制到项目层次结构中的某个位置。当然，您可以从头开始创建渲染脚本，但最好从默认脚本的副本开始，特别是如果您是 Defold 和/或图形编程的新手。

+ 编辑”default.render”文件的副本，并将#emph[Script]属性更改为引用您的渲染脚本副本。

+ 在#emph[game.project]设置文件中，将#emph[Render]属性（在#emph[bootstrap]下）更改为引用您的”default.render”文件副本。

== 渲染判定
<渲染判定>
为了能够控制对象的绘制顺序，您创建渲染\_判定\_。判定声明基于材质\_标签\_的选择应绘制什么。

每个绘制到屏幕上的对象都有一个附加的材质，该材质控制对象应如何在屏幕上绘制。在材质中，您指定一个或多个应与材质关联的\_标签\_。

在您的渲染脚本中，您可以创建一个#emph[渲染判定]并指定哪些标签应属于该判定。当您告诉引擎绘制判定时，每个具有包含为判定指定的所有标签的材质的对象都将被绘制。
...
=== Stretch 投影
<stretch-投影>
拉伸投影将始终绘制等于#emph[game.project]中设置的尺寸的游戏区域，即使窗口被调整大小。如果宽高比发生变化，将导致游戏内容在垂直或水平方向上被拉伸：

#box(image("../docs/en/manuals/images/render/stretch_projection.png"))

#emph[原始窗口大小的拉伸投影]

#box(image("../docs/en/manuals/images/render/stretch_projection_resized.png"))
...
=== Fixed fit 投影
<fixed-fit-投影>
就像拉伸投影一样，固定适配投影将始终显示等于#emph[game.project]中设置尺寸的游戏区域，但如果窗口被调整大小且宽高比发生变化，游戏内容将保持原始宽高比，并且将在垂直或水平方向上显示额外的游戏内容：

#box(image("../docs/en/manuals/images/render/fixed_fit_projection.png"))

#emph[原始窗口大小的固定适配投影]

#box(image("../docs/en/manuals/images/render/fixed_fit_projection_resized.png"))

#emph[窗口水平拉伸的固定适配投影]

#box(image("../docs/en/manuals/images/render/fixed_fit_projection_resized_smaller.png"))

#emph[窗口缩小到原始大小 50% 的固定适配投影]
...
=== Fixed 投影
<fixed-投影>
固定投影将保持原始宽高比，并以固定的缩放级别渲染游戏内容。这意味着如果缩放级别设置为不是 100% 的值，它将显示比#emph[game.project]中尺寸定义的游戏区域更多或更少的内容：

#box(image("../docs/en/manuals/images/render/fixed_projection_zoom_2_0.png"))

#emph[缩放设置为 2 的固定投影]

#box(image("../docs/en/manuals/images/render/fixed_projection_zoom_0_5.png"))

#emph[缩放设置为 0.5 的固定投影]

#box(image("../docs/en/manuals/images/render/fixed_projection_zoom_2_0_resized.png"))

#emph[缩放设置为 2 且窗口缩小到原始大小 50% 的固定投影]
...
=== 摄像机投影
<摄像机投影>
当使用默认渲染脚本且项目中有可用的已启用摄像机组件时，它们将优先于渲染脚本中设置的任何其他视图/投影。要了解有关在渲染脚本中使用摄像机组件的更多信息，请参阅摄像机文档。
...
/ on\_message(): #block[
渲染脚本可以定义一个 `on_message()` 函数并接收来自游戏或应用程序其他部分的消息。外部组件向渲染脚本发送信息的常见情况是\_摄像机\_。获得摄像机焦点的摄像机组件将自动每帧将其视图和投影发送到渲染脚本。此消息名为 `"set_view_projection"`：
]
...
#box(image("../docs/en/manuals/images/render/render_resources.png"))
...
== 系统消息
<系统消息>
/ `"set_view_projection"`: #block[
此消息从已获得摄像机焦点的摄像机组件发送。
]

/ `"window_resized"`: #block[
...
/ `"draw_line"`: #block[
绘制调试线。用于可视化 ray\_casts、向量等。线使用 `render.draw_debug3d()` 调用绘制。
]
...
== 绘制调用和批处理
<绘制调用和批处理>
绘制调用是描述使用纹理和材质以及可选的附加设置来设置 GPU 将对象绘制到屏幕的过程的术语。这个过程通常是资源密集型的，建议尽可能减少绘制调用的数量。您可以使用内置分析器来测量绘制调用的数量以及渲染它们所需的时间。
...
能够将节点排列成层次结构使得将节点分组为可管理的单元变得容易。但是，如果混合不同类型的节点，层次结构可能会有效地破坏批处理渲染。使用 GUI 层可以在保持节点层次结构的同时更有效地批处理 GUI 节点。您可以在GUI 手册中阅读有关 GUI 层以及它们如何影响绘制调用的更多信息。
