...
#box(image("../docs/en/manuals/images/camera/projections.png"))
...
要创建摄像机，右键点击一个游戏对象并选择Add Component ▸ Camera。您也可以在项目层次结构中创建一个组件文件，并将该组件文件添加到游戏对象中。



#box(image("../docs/en/manuals/images/camera/create.png"))



摄像机组件具有以下定义摄像机#emph[视锥体]的属性：



#box(image("../docs/en/manuals/images/camera/settings.png"))
...
(#strong[仅透视摄像机]) - 视锥体宽度和高度之间的比率。1.0表示您假设的是方形视图。1.33适用于4:3视图，如1024x768。1.78适用于16:9视图。如果设置了#emph[自动宽高比]，则此设置将被忽略。
...
(#strong[仅透视摄像机]) - 以\_弧度\_表示的摄像机#emph[垂直]视野。视野越宽，摄像机将看到的内容越多。
...
/ Auto Aspect Ratio: #block[

(#strong[仅透视摄像机]) - 设置此项以让摄像机自动计算宽高比。

]



/ Orthographic Projection: #block[
...
/ Orthographic Zoom: #block[

(#strong[仅正交摄像机]) - 用于正交投影的缩放（\> 1 = 放大，\< 1 = 缩小）。
...
要列出所有当前可用的摄像机，您可以使用camera.get\_cameras()：
...
脚本`camera`模块有多个可用于操作摄像机的函数。这里只是几个可以使用的函数，要查看所有可用函数，请查阅API文档中的手册。
...
+ 从摄像机组件发布的消息包括一个视图矩阵和一个投影矩阵。



摄像机组件根据摄像机的#emph[正交投影]属性为渲染脚本提供透视或正交投影矩阵。投影矩阵还考虑了定义的近和远裁剪平面、摄像机的视野和宽高比设置。



摄像机提供的视图矩阵定义了摄像机的位置和方向。具有#emph[正交投影]的摄像机将视图居中在它所附加的游戏对象的位置上，而具有#emph[透视投影]的摄像机将视图的左下角定位在它所附加的游戏对象上。
...
要检查摄像机是否处于活动状态，您可以使用摄像机API中的`get_enabled`函数：
...
使用正交摄像机时，您可以通过更改摄像机的#emph[正交缩放]属性来放大和缩小：
...
自适应缩放背后的概念是当显示分辨率从#emph[game.project]中设置的初始分辨率改变时调整摄像机缩放值。
...
+ 最大缩放 - 计算一个缩放值，使得#emph[game.project]中初始分辨率所覆盖的内容将填充并扩展到屏幕边界之外，可能会隐藏侧面或上方和下方的一些内容。

+ 最小缩放 - 计算一个缩放值，使得#emph[game.project]中初始分辨率所覆盖的内容将完全包含在屏幕边界内，可能会在侧面或上方和下方显示额外内容。
...
#box(image("../docs/en/manuals/images/camera/follow.png"))
...
请记住，来自`on_input()`的值`action.screen_x`和`action.screen_y`应该用作此函数的参数。访问示例页面以查看屏幕到世界坐标转换的实际操作。还有一个示例项目展示了如何进行屏幕到世界坐标的转换。



#block[

本手册中提到的第三方摄像机解决方案提供了用于屏幕坐标之间转换的函数。
...
您可以通过多种不同的消息和属性在运行时操作摄像机（请参阅API文档以了解用法）。
...
- 正交摄像机（仅2D）由Björn Ritzl开发。

- Defold Rendy（2D和3D）由Klayton Kowalski开发。
