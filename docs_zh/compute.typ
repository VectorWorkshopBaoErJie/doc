...
<计算程序>
#block[
Defold 中的计算着色器支持目前处于#emph[技术预览]状态。
...
== 创建计算程序
<创建计算程序>
要创建计算程序，在#emph[资源]浏览器中右键单击目标文件夹，然后选择新建… ▸ 计算。（您也可以从菜单中选择文件 ▸ 新建…，然后选择计算）。命名新的计算文件并按确定。

#box(image("../docs/en/manuals/images/compute/compute_file.png"))

新的计算将在#emph[计算编辑器]中打开。

#box(image("../docs/en/manuals/images/compute/compute.png"))

计算文件包含以下信息：

/ 计算程序: #block[
要使用的计算着色器程序文件（#emph[`.cp`]）。该着色器操作”抽象工作项”，这意味着输入和输出数据类型没有固定定义。程序员需要定义计算着色器应该产生什么。
]
...
#box(image("../docs/en/manuals/images/compute/compute_render_file.png"))
