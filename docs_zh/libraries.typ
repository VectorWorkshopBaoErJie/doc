...
== 设置库共享
<设置库共享>
假设您想要构建一个包含共享精灵和图块源的库。首先从设置新项目开始。确定您要从项目中共享哪些文件夹，并将这些文件夹的名称添加到项目设置中的#emph[`include_dirs`]属性中。如果您想列出多个文件夹，请用空格分隔名称：

#box(image("../docs/en/manuals/images/libraries/libraries_include_dirs.png"))
...
#box(image("../docs/en/manuals/images/libraries/libraries_library_url_github.png"))
...
#block[
建议在使用前始终审查第三方库。了解更多关于保护您对第三方软件的使用的信息。
...
==== GitHub认证
<github认证>
要从GitHub上的私有仓库获取，您需要生成个人访问令牌并将其用作您的密码。
...
==== GitLab认证
<gitlab认证>
要从GitLab上的私有仓库获取，您需要生成个人访问令牌并将其作为URL参数发送。
...
此解决方案在Defold论坛上提出并在此线程中讨论。

== 设置库依赖
<设置库依赖>
打开您想要访问库的项目。在项目设置中，将库URL添加到#emph[dependencies]属性中。如果需要，您可以指定多个依赖项目。只需使用`+`按钮一个一个地添加它们，并使用`-`按钮删除：

#box(image("../docs/en/manuals/images/libraries/libraries_dependencies.png"))

现在，选择Project ▸ Fetch Libraries来更新库依赖。每当您打开项目时，这都会自动发生，因此只有当依赖项更改而不重新打开项目时，您才需要这样做。如果您添加或删除依赖库，或者如果某个依赖库项目被某人更改并同步，就会发生这种情况。

#box(image("../docs/en/manuals/images/libraries/libraries_fetch_libraries.png"))

现在，您共享的文件夹出现在#emph[Assets]面板中，您可以使用您共享的所有内容。对库项目所做的任何同步更改都将在您的项目中可用。

#box(image("../docs/en/manuals/images/libraries/libraries_done.png"))

== 编辑库依赖中的文件
<编辑库依赖中的文件>
库中的文件无法保存。您可以进行更改，编辑器将能够使用这些更改进行构建，这对于测试很有用。但是，文件本身保持不变，当文件关闭时，所有修改都将被丢弃。

如果您想要更改库文件，请确保创建您自己的库分支并在那里进行更改。另一个选择是将整个库文件夹复制/粘贴到您的项目目录中并使用本地副本。在这种情况下，您的本地文件夹将遮蔽原始依赖项，并且应该从`game.project`中删除依赖项链接（不要忘记之后选择Project ▸ Fetch Libraries）。
...
如果您复制粘贴材质并希望跨某种类型的所有组件使用它，使用每个项目的模板可能很有用。
...
== 名称冲突
<名称冲突>
由于您可以在#emph[dependencies]项目设置中列出多个项目URL，您可能会遇到名称冲突。如果两个或更多依赖项目在#emph[`include_dirs`]项目设置中共享一个同名的文件夹，就会发生这种情况。

Defold通过简单地忽略除对同名文件夹的最后一个引用之外的所有引用来解决名称冲突，顺序是项目URL在#emph[dependencies]列表中指定的顺序。例如，如果您在依赖项中列出了3个库项目URL，并且它们都共享一个名为#emph[items]的文件夹，则只会显示一个#emph[items]文件夹------属于URL列表中最后一个项目的那个。
