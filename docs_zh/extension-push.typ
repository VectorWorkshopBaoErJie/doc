= 推送通知
<推送通知>
推送通知作为 原生扩展 在 iOS 和 Android 设备上用来给玩家提示修改和更新之类的信息。iOS 和 Android 实现方法基本一样但是也有一些平台相关的东西需要注意。
...
== 安装扩展
<安装扩展>
使用之前要在 #emph[game.project] 文件里配置扩展依赖。最新稳定版依赖的 URL 是：
...
<ios-配置>
#block[
为了更好地使用 Apple Push Notification 服务, 最好先熟读 Apple 的官方文档.
...
准备就绪后登录到 Apple 开发者中心。找到 AppID 打开通知推送服务。

#box(image("../docs/en/manuals/images/push/push_ios_app_id.png"))

记得创建推送服务 SSL 证书:

#box(image("../docs/en/manuals/images/push/push_ios_certificate.png"))

从你的服务器往通知推送服务器发送数据时需要这个证书。开发调试时，可以先在本地安装这个证书然后运行 APNS-Pusher 或者 NWPusher 之类的测试工具。

记得创建包含要推送通知 AppID 的供应商档案，并且在档案中设置好开发设备。可以在 "Member Center" 页面或者 Xcode 中进行这些操作。

#box(image("../docs/en/manuals/images/push/push_ios_provisioning_profile.png"))
...
#box(image("../docs/en/manuals/images/push/push_ios_pusher.png"))
...
#box(image("../docs/en/manuals/images/push/push_ios_notification.png"))
...
#block[
如果你的应用使用了 Google Cloud Messaging 平台，现在需要导入 Firebase Cloud Messaging 平台中。把应用从 Google dashboard 导入到 Firebase 即可。导入后 #emph[sender id] 保持不变，你的 #emph[game.project] 文件需要更新使用 `Firebase Application ID`（见下文）。

]
基本配置很简单。登录 #link("https://console.firebase.google.com")，选择或者新建应用。然后进入 #emph[Grow] 的 #emph[Cloud Messaging] 页面。

#box(image("../docs/en/manuals/images/push/push_fcm_add_cm.png"))

点击 Android 图标开始配置通知推送。

#box(image("../docs/en/manuals/images/push/push_fcm_configure_android.png"))

根据提示，输入 Defold 游戏项目使用的包名。

#box(image("../docs/en/manuals/images/push/push_fcm_register.png"))

下载 `google-services.json`，我们需要这个文件里面的一些信息。

#box(image("../docs/en/manuals/images/push/push_fcm_download_json.png"))

下面两步可以跳过，#emph[Add Firebase SDK] 和 #emph[Run your app to verify installation]。SDK 已打包进 Defold，无需手动添加。

用文本编辑器打开 `google-services.json` 文件，找到 #emph[`project_number`] 和 #emph[`mobilesdk_app_id`]。把这俩 id 考入 #emph[game.project] 文件，对应 #emph[android] 部分，`Gcm Sender Id`（`project_number`）和 `Fcm Application Id`（`mobilesdk_app_id`）。
...
#box(image("../docs/en/manuals/images/push/push_fcm_game_project.png"))

至此客户端准备就绪。上面测试 ios 时用的代码 对 Android 同样有效。运行并记录设备 token。
...
发送通知之前需要一个 Firebase 服务认证 key。这个 key 位于 Firebase dashboard 的 #emph[Settings] 的 #emph[Cloud Messaging] 页面中。

#box(image("../docs/en/manuals/images/push/push_fcm_server_key.png"))

一切准备就绪. 可以通过使用 Web API 的 #emph[curl] 命令测试推送功能:
...
注意 `seconds` 是指离通知推送还差多少秒. 还有一个函数可以查看当前安排下的 #emph[所有] 通知:
