...
集合代理与集合工厂组件不同。集合工厂将集合的内容实例化到当前游戏世界中。集合代理在运行时创建一个新的游戏世界，因此有不同的用例。

== 创建集合代理组件
<创建集合代理组件>
+ 通过右键点击游戏对象并从上下文菜单中选择Add Component ▸ Collection Proxy，将集合代理组件添加到游戏对象。

+ 将#emph[Collection]属性设置为您希望稍后动态加载到运行时中的集合的引用。该引用是静态的，确保所引用集合的所有内容最终都会出现在游戏中。

#box(image("../docs/en/manuals/images/collection-proxy/create_proxy.png"))

（您可以通过勾选#emph[Exclude]框并使用实时更新功能在构建中排除内容，然后通过代码下载。）

== 引导集合
<引导集合>
当Defold引擎启动时，它会从#emph[引导集合]加载并实例化所有游戏对象到运行时。然后初始化并启用游戏对象及其组件。引擎应该使用哪个引导集合是在项目设置中设置的。按照惯例，这个集合文件通常命名为”main.collection”。

#box(image("../docs/en/manuals/images/collection-proxy/bootstrap.png"))

为了容纳游戏对象及其组件，引擎为整个”游戏世界”分配所需的内存，引导集合的内容被实例化到这个世界中。还为任何碰撞对象和物理模拟创建了一个单独的物理世界。

由于脚本组件需要能够处理游戏中的所有对象，即使是来自引导世界之外的对象，它被赋予一个唯一的名称：您在集合文件中设置的#emph[Name]属性：

#box(image("../docs/en/manuals/images/collection-proxy/collection_id.png"))

如果加载的集合包含集合代理组件，那些代理引用的集合#emph[不会]被自动加载。您需要通过脚本控制这些资源的加载。
...
#box(image("../docs/en/manuals/images/collection-proxy/proxy_load.png"))

代理组件将指示引擎为新世界分配空间。还会创建一个单独的运行时物理世界，并且集合`"mylevel.collection"`中的所有游戏对象都被实例化。

新世界从集合文件中的#emph[Name]属性获取其名称，在本示例中设置为`"mylevel"`。名称必须是唯一的。如果在集合文件中设置的#emph[Name]已被用于已加载的世界，引擎将发出名称冲突错误：
...
== 定位到新世界
<定位到新世界>
在集合文件属性中设置的#emph[Name]用于定位已加载世界中的游戏对象和组件。例如，如果您在引导集合中创建了一个加载器对象，您可能需要从任何已加载的集合中与它通信：
...
#box(image("../docs/en/manuals/images/collection-proxy/message_passing.png"))

如果您需要从加载器与已加载集合中的游戏对象通信，您可以使用对象的完整URL发送消息：
...
== 时间步
<时间步>
集合代理更新可以通过改变\_时间步\_来进行缩放。这意味着即使游戏以稳定的60 FPS运行，代理也可以以更高或更低的速度更新，影响诸如：
...
有关更多详细信息，请参见`set_time_step`。

== 注意事项和常见问题
<注意事项和常见问题>
/ 物理: #block[
通过集合代理，可以将多个顶级集合或#emph[游戏世界]加载到引擎中。这样做时，重要的是要知道每个顶级集合都是一个单独的物理世界。物理交互（碰撞、触发、射线投射）只发生在属于同一世界的对象之间。因此，即使来自两个世界的碰撞对象在视觉上恰好彼此重叠，它们之间也不会有任何物理交互。
]

/ 内存: #block[
每个加载的集合都会创建一个新的游戏世界，这带来了相对较大的内存占用。如果您通过代理同时加载几十个集合，您可能需要重新考虑您的设计。要生成游戏对象层级的许多实例，集合工厂更适合。
]
