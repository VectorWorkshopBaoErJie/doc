= 游戏和系统日志
<游戏和系统日志>
游戏日志显示了引擎、原生扩展和游戏逻辑的所有输出。您可以从脚本和Lua模块中使用print()和pprint()命令在游戏日志中显示信息。您可以使用`dmLog`命名空间中的函数从原生扩展写入游戏日志。游戏日志可以从编辑器、终端窗口、使用平台特定工具或从日志文件中读取。
...
+ 您可以使用控制台工具读取游戏和系统日志。
+ 您可以使用LLDB调试器附加到设备上运行的游戏。要调试游戏，它需要使用包含您要调试的设备的”Apple Developer Provisioning Profile”进行签名。从编辑器打包游戏，并在打包对话框中提供配置文件（iOS打包仅在macOS上可用）。
...
== 从日志文件读取游戏日志
<从日志文件读取游戏日志>
如果您在#emph[game.project]中启用#emph[Write Log]设置，任何游戏输出都将写入磁盘，到一个名为”`log.txt`“的文件中。以下是在设备上运行游戏时如何提取文件的方法：

/ iOS: #block[
将设备连接到安装了macOS和Xcode的计算机上。
打开Xcode并转到Window ▸ Devices and Simulators。
在列表中选择您的设备，然后在#emph[Installed Apps]列表中选择相关的应用程序。
单击列表下方的齿轮图标并选择Download Container…。
#box(image("../docs/en/manuals/images/debugging/download_container.png"))

容器提取后，它将显示在#emph[Finder]中。右键单击容器并选择Show Package Content。找到文件”`log.txt`"，它应该位于"`AppData/Documents/`“中。
]

/ Android: #block[
提取”`log.txt`“的能力取决于操作系统版本和制造商。这里有一个简短的简单的逐步指南。
]
