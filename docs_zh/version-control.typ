= 版本控制
<版本控制>
Defold 是为密切合作的小型游戏团队而设计的，团队成员可以在同一内容上并行工作，几乎没有任何摩擦。Defold 内置支持使用 Git 进行版本控制。Git 专为分布式协作工作而设计，是一款极其强大的工具，支持广泛的工作流程。

== 已更改的文件
<已更改的文件>
当您在本地工作副本中保存更改时，Defold 会在 #emph[Changed Files] 编辑器面板中跟踪所有更改，列出每个已添加、删除或修改的文件。

#box(image("../docs/en/manuals/images/workflow/changed_files.png"))

在列表中选择一个文件，然后点击 Diff 查看您对文件所做的更改，或点击 Revert 撤销所有更改并将文件恢复到最后一次同步后的状态。
...
+ 任何本地更改都会被暂存，以便在同步过程中稍后出现问题时可以恢复。
+ 服务器更改会被拉取。
+ 应用暂存（恢复本地更改），这可能会导致需要解决的合并冲突。
+ 用户可以选择提交任何本地文件更改。
+ 如果有本地提交，用户可以选择将这些提交推送到服务器。同样，这可能会导致需要解决的冲突。
