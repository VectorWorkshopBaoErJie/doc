...
Flash 游戏使用 ActionScript 编程（3.0 是最新版本），而 Defold 脚本使用 Lua 编写。本指南不会详细比较 Lua 和 Actionscript 3.0。Defold 手册 提供了 Defold 中 Lua 编程的良好介绍，并引用了非常有用的 Programming in Lua（第一版），该书可在线免费获取。

Jesse Warden 的一篇文章提供了 Actionscript 和 Lua 的基本比较，这可以作为一个很好的起点。但请注意，Defold 和 Flash 在构建方式上的差异比语言层面可见的差异更深。Actionscript 和 Flash 在经典意义上是面向对象的，具有类和继承。Defold 没有类，也没有继承。它包含 #emph[游戏对象] 的概念，可以包含视听表示、行为和数据。对游戏对象的操作是通过 Defold API 中可用的 #emph[函数] 完成的。此外，Defold 鼓励使用 #emph[消息] 在对象之间进行通信。消息是比方法调用更高级的构造，并不打算用作方法调用。这些差异很重要，需要一段时间才能适应，但本指南不会详细讨论。
...
#box(image("../docs/en/manuals/images/flash/go_movieclip.png"))

不同的是，Defold 游戏对象没有时间轴。却能包含很多组件。组件有 sprite、sound、脚本等等（关于组件详情请见 构成教程）。下图这个游戏对象包含一个 sprite 和一个脚本。脚本用来控制游戏对象生命周期中的行为：

#box(image("../docs/en/manuals/images/flash/script_component.png"))

影片剪辑可以包含其他影片剪辑，游戏对象不是 #emph[包含] 其他游戏对象。但是能够与其他游戏对象建立 #emph[父子] 层级关系，父子关系的游戏对象可以一起移动、旋转和缩放。

== Flash 手动创建影片剪辑
<flash-手动创建影片剪辑>
在 Flash 中，影片剪辑可以在 IDE 中手动创建，也可以运行时动态创建。手动创建的影片剪辑需要给实例名，才能在代码里引用。

#box(image("../docs/en/manuals/images/flash/manual_movie_clips.png"))

== Defold---手动创建游戏对象
<defold手动创建游戏对象>
Defold 中，游戏对象可以在编辑器里手动创建，也可以运行时动态创建。手动创建的游戏对象需要给唯一 id，才能在代码里引用。

集合可以用来管理游戏对象。集合是容纳游戏对象和其他集合的容器（或称 prefabs）。最简单的情况，一个游戏有一个集合。通常，Defold 游戏包含许多集合，或者手动指定启动 “main” 集合或者通过 集合代理 动态载入集合。但是 Flash 的 "levels" 或者 "screens" 没有这个能力。

下面的例子里，"main" 集合（看右边，#emph[Outline] 窗口里）包含3个 "logo" 游戏对象（看左边，#emph[Assets] 浏览器窗口里）：

#box(image("../docs/en/manuals/images/flash/manual_game_objects.png"))

== Flash---手动引用影片剪辑
<flash手动引用影片剪辑>
Flash 需要定义影片剪辑实例名再手动引用:

#box(image("../docs/en/manuals/images/flash/flash_instance_name.png"))
...
手动拖放对象的地址由 #emph[Id] 属性 (上图右下角) 决定. 每个集合里一个对象的id是唯一的. 编辑器可以自动生成默认id但是所有对象的id都可以随意更改.

#box(image("../docs/en/manuals/images/flash/game_object_id.png"))

#block[
对象的id可以使用脚本: `print(go.get_id())` 查看. 它会在控制台打印出当前游戏对象的id.

]
地址定位和消息传递是 Defold 游戏开发的核心概念. 定位教程 和 消息传递教程 里有更详细的介绍.

== Flash---动态创建影片剪辑
<flash动态创建影片剪辑>
Flash 里动态创建影片剪辑, 需要预先设置好 ActionScript Linkage:

#box(image("../docs/en/manuals/images/flash/actionscript_linkage.png"))
...
== Defold---使用工厂创建游戏对象
<defold使用工厂创建游戏对象>
Defold 使用 #emph[工厂] 动态创建游戏对象. 工厂是创建游戏对象拷贝的组件. 本例中, 以 "logo" 游戏对象为原型创建了一个工厂组件:

#box(image("../docs/en/manuals/images/flash/logo_factory.png"))

注意工厂组件, 需要像其他组件一样, 需要添加到游戏对象里才能用. 本例中, 我们创建了叫做 "factories" 的游戏对象, 来容纳工厂组件:

#box(image("../docs/en/manuals/images/flash/factory_component.png"))
...
#box(image("../docs/en/manuals/images/flash/stage.png"))

如上面影片剪辑部分所讨论的，舞台本质上是 Flash 游戏的顶级容器，在每次导出项目时创建。舞台默认情况下有一个子对象，即 #emph[`MainTimeline`]。项目中生成的每个影片剪辑都有自己的时间轴，并且可以作为其他符号（包括影片剪辑）的容器。

== Defold---collections
<defoldcollections>
Defold 的集合类似于舞台. 引擎启动时集合文件的内容组成了游戏世界. 默认启动集合叫 "main.collection" 但是可以在 #emph[game.project] 项目配置文件里随意更改:

#box(image("../docs/en/manuals/images/flash/game_project.png"))

集合作为容器管理着游戏对象和其他集合. 通过 集合工厂 可以在运行时动态创建集合内容, 就像游戏对象工厂创建游戏对象一样. 集合可以包含多组敌人, 或者一堆钱币, 之类的. 下图中, 我们手动拖放了两组 "logos" 集合到 "main" 集合中.

#box(image("../docs/en/manuals/images/flash/collection.png"))
...
#box(image("../docs/en/manuals/images/flash/timeline.png"))

运动补间允许对对象的各种属性进行动画，包括大小、位置和旋转。在下面的例子中，所有列出的属性都被修改了。

#box(image("../docs/en/manuals/images/flash/tween.png"))
...
#box(image("../docs/en/manuals/images/flash/flipbook.png"))
...
#box(image("../docs/en/manuals/images/flash/depth_index.png"))
...
#box(image("../docs/en/manuals/images/flash/depth_index_2.png"))

== Defold---z position
<defoldz-position>
Defold 使用 z 轴位置控制游戏对象的显示顺序。每个游戏对象的位置向量包含 x、y、z 三个分量，其中 z 值越大，对象越靠前显示。在默认 渲染脚本 中，z 轴有效范围为 -1 到 1。

#block[
若游戏对象的 z 值超出 \[-1, 1\] 范围将不会被渲染（不可见）。这是新手常见困惑点，若对象未显示请优先检查 z 值。

]
与 Flash 的深度索引不同，Flash 编辑器只隐含深度索引（并允许使用#emph[Bring Forward]和#emph[Send Backward]等命令修改），而 Defold 可直接在编辑器中设置 z 值。下图示例中，"logo3" 因 z=0.2 显示在最上层，其余两个对象 z 值分别为 0.0 和 0.1：

#box(image("../docs/en/manuals/images/flash/z_order.png"))

层级叠加规则：对象的最终 z 值 = 自身 z 值 + 所有父级 z 值之和。例如，若 "logos" 集合（包含三个图标）的 z=0.9，则三个图标的最终 z 值为 0.9、1.0、1.1，此时 "logo3" 因 z=1.1 \> 1 而不可见：

#box(image("../docs/en/manuals/images/flash/z_order_outline.png"))
...
#box(image("../docs/en/manuals/images/flash/hittest.png"))
...
#box(image("../docs/en/manuals/images/flash/hitboundingbox.png"))
...
<defoldcollision-objects>
Defold 内含物理引擎可以用于碰撞检测然后使用其上的脚本进行响应。首先要在游戏对象上面添加碰撞对象组件。如下图所示，我们对 "bullet" 游戏对象添加了碰撞对象。碰撞对象以红色半透明方块表示（只在编辑器中可见）：

#box(image("../docs/en/manuals/images/flash/collision_object.png"))

Defold 包含一个 Box2D 物理引擎的修改版，可以用来自动模拟真实的碰撞。本教程使用 Kinematic 碰撞对象，因为它的碰撞检测和 Flash 的最接近。关于动态碰撞详情请见 Defold 物理教程。

此碰撞对象包含如下属性：

#box(image("../docs/en/manuals/images/flash/collision_object_properties.png"))
...
#box(image("../docs/en/manuals/images/flash/collision_groupmask.png"))

子弹之间就能相互碰撞了。我们为靶子设置了如下的碰撞对象：

#box(image("../docs/en/manuals/images/flash/collision_object_bullet.png"))

注意 #emph[Group] 属性设置为了 "target" 然后 #emph[Mask] 设置为了 "bullet"。
...
<defoldcall-back-functions-and-messaging>
...
/ on\_input: #block[
当用户输入（例如鼠标或键盘）发送到具有输入焦点的游戏对象时调用，这意味着该对象接收所有输入并可以对其做出反应。
]
...
上面列出的回调函数都是可选的，如果不使用可以删除。有关如何设置输入的详细信息，请参阅输入手册。使用集合代理时会出现一个常见问题 - 请参阅输入手册的此部分获取更多信息。

如碰撞检测部分所述，碰撞事件通过向涉及的游戏对象发送消息来处理。它们各自的脚本组件在其 on\_message 回调函数中接收消息。

== Flash---按钮符号
<flash按钮符号>
Flash 使用专用的按钮符号类型。按钮使用特定的事件处理方法（如 `click` 和 `buttonDown`）在检测到用户交互时执行动作。按钮符号中“点击”部分的图形形状决定了按钮的点击区域。

#box(image("../docs/en/manuals/images/flash/button.png"))

== Defold---GUI 场景和脚本
<defoldgui-场景和脚本>
Defold 不包含原生按钮组件，也不能像 Flash 中那样轻易地检测对特定游戏对象形状的点击。使用 GUI 组件是最常见的解决方案，部分原因是 Defold GUI 组件的位置不受游戏内摄像机影响（如果使用的话）。GUI API 还包含检测用户输入（如点击和触摸事件）是否在 GUI 元素边界内的函数。
...
#box(image("../docs/en/manuals/images/flash/no_debug.png"))

启用物理调试后，项目中的碰撞对象会显示出来：

#box(image("../docs/en/manuals/images/flash/with_debug.png"))

当碰撞发生时，相关碰撞对象会高光显示。而且，碰撞向量也会被显示出来：

#box(image("../docs/en/manuals/images/flash/collision.png"))
...
如果你有问题或遇到困难，Defold 论坛 是寻求帮助的好地方。
