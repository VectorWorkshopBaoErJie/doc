...
#box(image("../docs/en/manuals/images/instant-games/instantgames.png"))
...
+ 在你的 #emph[game.project] 文件中添加 Instant Games 扩展依赖。即把 "https:\/\/github.com/defold/extension-fbinstant/archive/master.zip" 添加到 #emph[Dependencies] 属性中。

  #box(image("../docs/en/manuals/images/instant-games/game_project.png"))
...
关于如何在 Defold 中使用 Instant Games SDK 详情请见 API 文档。

== 示例游戏
<示例游戏>
有个 Defold 游戏 "Tic Tac Toe" 的 Instant Games 版本在 官方 GitHub 代码库 提供了完整的源码和素材。可以作为学习资料或者素材来使用。

#box(image("../docs/en/manuals/images/instant-games/tictactoe.png"))

== 减小包体
<减小包体>
Facebook Instant Games 最佳实践 建议游戏首次载入时间不得超过 5 秒。这对 Defold 来说有点勉强，好在还有一些方法能减小游戏体积：

/ 去掉未使用引擎特性: #block[
HTML5 版的标准 Defold 引擎用 gzip 压缩后小于 1.2MB，但是可以通过去掉不必要的引擎特效来达到减少包体的效果。在 "app.manifest" 文件中可以指定去掉引擎哪方面特性，此文件在 #emph[game.project] 文件的 Native Extension 部分 引用。
此功能尚处于测试阶段且没有说明文档。这里：https:\/\/forum.defold.com/t/stripping-appmanifest-maker/16059 有一个可以自动生成 app.manifests 文件的工具。
]

/ 使用纹理压缩: #block[
纹理压缩是有效减少包体和内存占用的方法。关于纹理压缩详情请见 Texture Profiles 教程。
]
