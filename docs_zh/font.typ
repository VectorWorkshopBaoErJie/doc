...
== 创建字体
<创建字体>
要在Defold中创建字体，请从菜单中选择文件 ▸ 新建…，然后选择字体。您也可以在#emph[资源]浏览器中右键点击一个位置，然后选择新建… ▸ 字体。

#box(image("../docs/en/manuals/images/font/new_font_name.png"))

给新字体文件命名并点击确定。新字体文件现在在编辑器中打开。

#box(image("../docs/en/manuals/images/font/new_font.png"))

将您想要使用的字体拖放到#emph[资源]浏览器中，并将其放在合适的位置。

将#emph[字体]属性设置为您想要使用的字体文件，并根据需要设置字体属性。

== 属性
<属性>
/ #emph[字体]: #block[
用于生成字体数据的TTF、OTF或#emph[`.fnt`]文件。
]
...
/ #emph[输出格式]: #block[
生成的字体数据类型。
- `TYPE_BITMAP` 将导入的OTF或TTF文件转换为字体表纹理，其中位图数据用于渲染文本节点。颜色通道用于编码字体形状、轮廓和阴影。对于#emph[`.fnt`]文件，直接使用源纹理位图。
...
#block[
阴影支持由内置字体材质着色器启用，并处理单层和多层渲染模式。如果您不需要分层字体渲染或阴影支持，最好使用更简单的着色器，如#emph[`builtins/font-singlelayer.fp`]。
...
#box(image("../docs/en/manuals/images/font/df_font.png"))

确保在创建字体时将字体的#emph[材质]属性更改为#emph[`builtins/fonts/font-df.material`]（或任何其他可以处理距离场数据的材质）---否则字体在渲染到屏幕时将不会使用正确的着色器。

== 位图BMFonts
<位图bmfonts>
除了生成的位图外，Defold还支持预烘焙的位图”BMFont”格式字体。这些字体由包含所有字形的PNG字体表组成。此外，#emph[`.fnt`]文件包含有关在表上可以找到每个字形的位置以及大小和字距调整信息的信息。（请注意，Defold不支持Phaser和其他一些工具使用的#emph[`.fnt`]格式的XML版本）

这些类型的字体与从TrueType或OpenType字体文件生成的位图字体相比没有性能改进，但可以在图像中直接包含任意图形、颜色和阴影。

将生成的#emph[`.fnt`]和#emph[`.png`]文件添加到您的Defold项目中。这些文件应该位于同一文件夹中。创建一个新的字体文件，并将#emph[字体]属性设置为#emph[`.fnt`]文件。确保#emph[output\_format]设置为`TYPE_BITMAP`。Defold不会生成位图，而是使用PNG中提供的位图。

#block[
要创建BMFont，您需要使用能够生成适当文件的工具。存在几个选项：

- Bitmap Font Generator，AngelCode提供的仅限Windows的工具。
- Shoebox，免费的基于Adobe Air的应用程序，适用于Windows和macOS。
- Hiero，基于Java的开源工具。
- Glyph Designer，71 Squared的商业macOS工具。
- bmGlyph，Sovapps的商业macOS工具。

]
#box(image("../docs/en/manuals/images/font/bm_font.png"))

为了使字体正确渲染，不要忘记在创建字体时将材质属性设置为#emph[`builtins/fonts/font-fnt.material`]。
