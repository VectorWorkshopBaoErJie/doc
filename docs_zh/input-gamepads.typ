#block[
建议您先熟悉Defold中输入的一般工作方式，如何接收输入以及输入在脚本文件中的接收顺序。有关输入系统的更多信息，请参阅输入系统概述手册。
...
#box(image("../docs/en/manuals/images/input/gamepad_bindings.png"))
...
== 游戏手柄设置文件
<游戏手柄设置文件>
游戏手柄输入设置为每种硬件游戏手柄类型使用单独的映射文件。特定硬件游戏手柄的游戏手柄映射在#emph[gamepads]文件中设置。Defold附带了一个内置的游戏手柄文件，其中包含常见游戏手柄的设置：

#box(image("../docs/en/manuals/images/input/gamepads.png"))
...
该工具将要求您按下连接控制器上的不同按钮。然后，它将输出一个新的游戏手柄文件，其中包含您控制器的正确映射。保存新文件，或将其与现有的游戏手柄文件合并，然后在#emph[game.project]中更新设置：

#box(image("../docs/en/manuals/images/input/gamepad_setting.png"))
...
== HTML5中的游戏手柄
<html5中的游戏手柄>
HTML5构建中支持游戏手柄，并生成与其他平台相同的输入事件。游戏手柄的支持基于游戏手柄API，大多数浏览器都支持此API（参考此支持图表）。如果浏览器不支持游戏手柄API，Defold将静默忽略项目中的任何游戏手柄触发器。您可以通过检查`navigator`对象上是否存在`getGamepads`函数来检查浏览器是否支持游戏手柄API：
...
如果连接的游戏手柄被浏览器识别为标准游戏手柄，它将使用游戏手柄设置文件中的”Standard Gamepad”映射（在`/builtins`中的`default.gamepads`文件中包含标准游戏手柄映射）。标准游戏手柄定义为具有16个按钮和2个模拟摇杆，按钮布局类似于PlayStation或Xbox控制器（有关更多信息，请参阅W3C定义和按钮布局）。如果连接的游戏手柄未被识别为标准游戏手柄，Defold将在游戏手柄设置文件中查找与硬件游戏手柄类型匹配的映射。
...
Android构建中支持游戏手柄，并生成与其他平台相同的输入事件。游戏手柄的支持基于Android按键和运动事件输入系统。Android输入事件将使用上述相同的#emph[gamepad]文件转换为Defold游戏手柄事件。

在Android上添加额外的游戏手柄绑定时，您可以使用以下查找表将Android输入事件转换为#emph[gamepad]文件值：

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([按键事件到按钮索引], [索引], [版本],),
    table.hline(),
