...
编辑器启动时会加载您的自定义样式表并将其应用在默认样式之上。编辑器使用 JavaFX 编写用户界面，所以样式表几乎等价于浏览器中用于网页的 CSS 文件。官方默认的样式表保存于 GitHub 上。

== 修改颜色
<修改颜色>
默认颜色在 `_palette.scss` 中定义, 类似如下设置:
...
#box(image("../docs/en/manuals/images/editor/editor-styling-color.png"))

== 修改字体
<修改字体>
编辑器使用两种字体：代码编写和等宽文本（错误信息）使用`Dejavu Sans Mono`，其他UI使用`Source Sans Pro`。字体定义主要保存在`_typography.scss`中，如下所示：
...
#box(image("../docs/en/manuals/images/editor/editor-styling-fonts.png"))
