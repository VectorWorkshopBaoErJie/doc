...
== 精灵属性
<精灵属性>
除了 #emph[Id]、#emph[Position] 和 #emph[Rotation] 属性外，还有以下组件特定属性：
...
/ `image`: #block[
精灵图像（`hash`）。您可以使用图集或瓷砖图源资源属性和 `go.set()` 来更改此属性。请参阅API 参考中的示例。
]

/ `material`: #block[
精灵材质（`hash`）。您可以使用材质资源属性和 `go.set()` 来更改此属性。请参阅API 参考中的示例。
]
...
#box(image("../docs/en/manuals/images/graphics/sprite-attributes.png"))

== 项目配置
<项目配置>
#emph[game.project] 文件中有一些与精灵相关的项目设置。
