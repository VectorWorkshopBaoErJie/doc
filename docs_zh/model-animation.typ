...
#box(image("../docs/en/manuals/images/animation/blender_animation.png"))
#box(image("../docs/en/manuals/images/animation/suzanne.gif"))

== 播放动画
<播放动画>
模型使用`model.play_anim()`函数进行动画处理：
...
不支持 Collada 中的动画剪辑。要对每个模型使用多个动画，请将它们导出到单独的 #emph[.dae] 文件中，然后在 Defold 中将这些文件收集到一个 #emph[.animationset] 文件中。
...
您可以在运行时检索骨骼游戏对象的实例 id。函数`model.get_go()`返回指定骨骼的游戏对象的 id。
...
=== 游标动画
<游标动画>
除了使用`model.play_anim()`来推进模型动画外，#emph[Model]组件还公开了一个”游标”属性，可以使用`go.animate()`进行操作（有关属性动画的更多信息）：
