...
== 安装扩展
<安装扩展>
在你的 #emph[game.project] 文件中设置 webview 依赖。
最新版本位于如下 URL：
...
- #strong[`self`] - 回调函数所处的脚本引用。
- #strong[`webview_id`] - 回调函数可以做到多个 webview 共用，这个参数引用的是回调事件发生的那个 webview 的 id。
- #strong[`request_id`] - 同样 `webview.open` 也可以共用一个回调回调函数，这个参数引用的是回调事件发生的那个载入请求的 id。
- #strong[`type`] - 事件的类型，枚举类型，下文 #emph[回调类型] 章节会详细探讨。
- #strong[`data`] - 不同时间连带的各种数据信息。

==== 回调类型
<回调类型>
回调的 `type` 参数可以被设置为以下枚举之一：

- #strong[`webview.CALLBACK_RESULT_URL_LOADING`] - 在 webview 内导航时，载入事件被触发。调用 `webview.open` 或者
  用户点击页面上的超级链接都会造成事件触发。事件的处理结果决定了载入请求是否被允许。如果返回 `false` 则载入终止，其他值则是允许载入。
  被允许的链接也会记录在回调函数 `data` 表的 `url` 项里。
- #strong[`webview.CALLBACK_RESULT_URL_ERROR`] 和 #strong[`webview.CALLBACK_RESULT_EVAL_ERROR`] - 一旦载入出错，或者
  执行 JavaScript 脚本出错（下文 #emph[运行 JavaScript 脚本] 章节会详细探讨），错误事件就会被触发。`data` 表的 `result` 项用一个字符串记录了错误的详细内容。
- #strong[`webview.CALLBACK_RESULT_URL_OK`] 和 #strong[`webview.CALLBACK_RESULT_EVAL_OK`] - 一旦载入成功，或者执行 JavaScript 脚本成功，
...
#box(image("../docs/en/manuals/images/webview/webview_player_feedback1.png"))

注意：我们的例子里有一个指向 google.com 的超级链接，目的是试试玩家点别的链接出不出的去。------牛虻

强制反馈功能完成！但是怎么提交反馈呢？也许玩家不想提交反馈呢，怎么返回游戏？再说，全屏的网页真的合适吗？后面的游戏界面全都被挡住了啊！
我们会在下文的 #emph[运行 JavaScript 脚本] 和 #emph[可视定位控制面板] 章节继续探讨。
...
#strong[注意：] 本例中 `webview_callback` 函数与前例类似故忽略。

众所周知一个 HTML 源码包含所有代码和资源是不明智的，最好把 JavaScript 和 CSS 等资源分门别类保存为文件，这种情况下
可以使用 `sys.load_resource` 进行资源文件的预加载。
...
#box(image("../docs/en/manuals/images/webview/webview_player_feedback2.png"))
...
注意这里 `webview.eval` 返回的并不是 JavaScript 的运行结果，而是 #emph[运行请求 id]。我们首先要确定回调函数的请求 id，这个函数里传入的 `data.result` 才是真正的 JavaScript 运行结果。
