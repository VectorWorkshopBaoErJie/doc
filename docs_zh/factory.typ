...
当您将 Factory 组件添加到游戏对象时，您可以在 #emph[Prototype] 属性中指定工厂应使用哪个游戏对象文件作为原型（在其他引擎中也称为”预制件”或”蓝图”）来创建所有新的游戏对象。

#box(image("../docs/en/manuals/images/factory/factory_collection.png"))

#box(image("../docs/en/manuals/images/factory/factory_component.png"))
...
#box(image("../docs/en/manuals/images/factory/factory_spawned.png"))
...
/ `[properties]`: #block[
（可选）一个 Lua 表，包含任何用于初始化游戏对象的脚本属性值。有关脚本属性的信息，请参阅脚本属性手册。
]
...
+ "score"脚本属性定义为默认值。
+ 将”score”脚本属性引用为存储在”self”中的值。

#box(image("../docs/en/manuals/images/factory/factory_spawned2.png"))
...
== Factory 创建对象的寻址
<factory-创建对象的寻址>
Defold 的寻址机制使得可以访问运行中游戏中的每个对象和组件。寻址手册详细介绍了系统的工作原理。可以对生成的游戏对象及其组件使用相同的寻址机制。通常，使用生成对象的 ID 就足够了，例如在发送消息时：
...
== Factory 资源的动态加载
<factory-资源的动态加载>
通过勾选工厂属性中的 #emph[Load Dynamically] 复选框，引擎会推迟与工厂关联的资源的加载。

#box(image("../docs/en/manuals/images/factory/load_dynamically.png"))
...
/ 同步加载: #block[
当您想要生成对象时调用 `factory.create()`。这将同步加载资源，可能会导致卡顿，然后生成新实例。
```lua
function init(self)
    -- 当工厂的父级集合加载时，
...
/ 异步加载: #block[
调用 `factory.load()` 显式异步加载资源。当资源准备好生成时，会收到一个回调。
```lua
...
== 动态原型
<动态原型>
通过勾选工厂属性中的 #emph[Dynamic Prototype] 复选框，可以更改工厂可以创建的 #emph[Prototype]。

#box(image("../docs/en/manuals/images/factory/dynamic_prototype.png"))

当 #emph[Dynamic Prototype] 选项被勾选时，工厂组件可以使用 `factory.set_prototype()` 函数更改原型。示例：
...
#block[
当设置 #emph[Dynamic Prototype] 选项时，无法优化集合组件计数，拥有集合将使用 #emph[game.project] 文件中的默认组件计数。

]
== 实例限制
<实例限制>
项目设置 #emph[Collection related settings] 中的 #emph[max\_instances] 限制了世界中可以存在的游戏对象实例总数（启动时加载的 main.collection 或通过集合代理加载的任何世界）。世界中存在的所有游戏对象都计入此限制，无论它们是通过编辑器手动放置的还是通过脚本在运行时生成的。

#box(image("../docs/en/manuals/images/factory/factory_max_instances.png"))

如果您将 #emph[max\_instances] 设置为 1024 并在主集合中有 24 个手动放置的游戏对象，您可以额外生成 1000 个游戏对象。一旦删除一个游戏对象，您就可以自由生成另一个实例。
