= Flip-book animation
<flip-book-animation>
逐帧动画由一系列静态图像连续显示组成。这种技术非常类似于传统的赛璐珞动画（参见 http:\/\/en.wikipedia.org/wiki/Traditional\_animation）。由于每一帧都可以单独操作，该技术提供了无限的可能性。然而，由于每一帧都存储在独特的图像中，内存占用可能会很高。动画的平滑度也取决于每秒显示的图像数量，但增加图像数量通常也会增加工作量。Defold 逐帧动画要么作为添加到图集中的单独图像存储，要么作为瓷砖图源存储，所有帧以水平序列排列。

#box(image("../docs/en/manuals/images/animation/animsheet.png"))
#box(image("../docs/en/manuals/images/animation/runloop.gif"))
...
/ 精灵: #block[
要在运行时运行动画，您可以使用 `sprite.play_flipbook()` 函数。请参见下面的示例。
]

/ GUI方块节点: #block[
要在运行时运行动画，您可以使用 `gui.play_flipbook()` 函数。请参见下面的示例。
]

#block[
ping-pong 播放模式会将动画播放到最后一帧，然后反转顺序并播放回动画的#strong[第二帧]，而不是回到第一帧。这样做是为了使动画链接更容易。
